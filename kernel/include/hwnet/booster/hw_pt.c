...
{
	return (((g_hw_pt_ul_abnormal_data_count + g_hw_pt_dl_in_abnormal_data_count) > KERNEL_ABNORMAL_PACK_CONTS)
			|| (g_hw_pt_ul_abnormal_data_count > KERNEL_ABNORMAL_PACK_CONTS)
			|| (g_hw_pt_dl_in_abnormal_data_count > KERNEL_ABNORMAL_PACK_CONTS)
			|| (g_hw_pt_dl_out_abnormal_data_count > RECV_QUEUE_ABNORMAL_PACK_CONTS));
}
...
static void prepare_report_data(struct hw_kernel_delay_info *info_rpt)
{
	...
	if (g_hw_pt_ul_abnormal_data_count == 0)
		...
		info_rpt->uplink_time_cost_ave = g_hw_pt_ul_data_time_cost / g_hw_pt_ul_abnormal_data_count;
	...
	if (g_hw_pt_dl_in_abnormal_data_count == 0)
		...
		info_rpt->downlink_time_cost_ave = g_hw_pt_dl_data_time_cost / g_hw_pt_dl_in_abnormal_data_count;

	info_rpt->downlink_out_wait_cnt = g_hw_pt_dl_out_abnormal_data_count;
	info_rpt->uplink_abnormal_not_report_cnt = g_hw_pt_ul_abnormal_data_not_rpt_count;
	info_rpt->downlink_abnormal_not_report_cnt = g_hw_pt_dl_abnormal_data_not_rpt_count;
}
...
void hw_pt_dev_uplink(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (time_cost < 0)
		return;
	time_cost = jiffies_to_msecs(time_cost);

	switch (get_layer4_protocl(skb)) {
	case IPPROTO_TCP:
		th = tcp_hdr(skb);
		if (th != NULL) {
			package_len = th->doff*4;
