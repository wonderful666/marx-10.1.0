			  ...
			  gfp_t gfp_mask, struct mem_cgroup **memcgp, bool compound)
{
	...
	unsigned int nr_pages = compound ? hpage_nr_pages(page) : 1;
	int ret = 0;
	struct page_counter *counter = NULL;
	unsigned long limit;
	...
	if (PageProtect(page)) {
		...
		if (is_prot_memcg(memcg, false) && !limit) {
			ClearPageProtect(page);
			memcg = NULL;
		}
...
int protect_max_mbytes_handler(struct ctl_table *table, int write,
		void __user *buffer, size_t *length, loff_t *ppos)
{
	int ret = 0;
	int i = 0;
	...
	for (; i < PROTECT_MEMCG_MAX; i++) {
		if (!prot_mem_cgroup[i])
			continue;

		if (!protect_max_mbytes[i] && protect_lru_enable)
			protect_lru_enable = 0;

		if (!protect_max_mbytes[i])
			drain_all_stock(prot_mem_cgroup[i]);
		lru_add_drain_all();
		ret = mem_cgroup_resize_limit(prot_mem_cgroup[i],
				protect_max_mbytes[i] << (20 - PAGE_SHIFT));
	...
	for (; i < PROTECT_MEMCG_MAX; i++) {
		counter = &prot_mem_cgroup[i]->memory;
		protect_max_mbytes[i] = counter->limit >> (20 - PAGE_SHIFT);
	}

	return ret;
}

int protect_cur_mbytes_handler(struct ctl_table *table, int write,
		void __user *buffer, size_t *length, loff_t *ppos)
{
	int i = 0;
	...
	for (; i < PROTECT_MEMCG_MAX; i++) {
		if (prot_mem_cgroup[i]) {
			protect_cur_mbytes[i] = mem_cgroup_usage(
