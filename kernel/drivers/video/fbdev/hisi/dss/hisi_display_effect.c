...
static int hisifb_effect_info_set_handler(const void __user *argp)
{
	...
	if (effect_info.modules & DSS_EFFECT_MODULE_DPPROI) {
		hisi_effect_dpproi_verify(hisifd_primary, &effect_info);
		ret = hisi_effect_dpproi_info_set(hisifd_primary,
...
#if defined(CONFIG_HISI_FB_V510)
		ret = hisi_effect_dpp_info_set(hisifd_primary, &effect_info);
#else
	...
	dss_overlay_t *pov_req)
{
	...
	if (pov_req->hiace_roi_support) {
		...
		start_point = ((uint32_t)rect->y << 16) | ((uint32_t)rect->x +
			hisifd->panel_info.dummy_pixel_num / 2);
