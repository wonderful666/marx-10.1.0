...
static uint32_t get_current_mipi_task(struct hisi_fb_data_type *hisifd)
{
	...
	if (hisifd->panel_info.frm_rate_ctrl.status == FRM_UPDT_NEED_DOING ||
		hisifd->panel_info.frm_rate_ctrl.status == FRM_UPDT_DOING)
		mipi_task |= BIT(2);

	return mipi_task;
}

void hisifb_wait_for_mipi_resource_available(struct hisi_fb_data_type *hisifd)
{
	int wait_count = 0;
	uint32_t task;
	uint32_t task_tmp;
	...
	task = get_current_mipi_task(hisifd);
	...
	if (task != 0) {
		...
		while ((task_tmp != 0) && (wait_count < MIPI_CLK_UPDT_TIMEOUT)) {
			...
			task_tmp = get_current_mipi_task(hisifd);
		}
