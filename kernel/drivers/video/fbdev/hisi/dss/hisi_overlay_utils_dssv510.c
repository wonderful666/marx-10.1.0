...
static uint32_t vactive_timeout_count;
	...
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block)
{
	...
	for (i = 0; i < pov_h_block->layer_nums; i++) {
		...
		if ((layer->need_cap & CAP_AFBCD) || (layer->need_cap & CAP_HFBCD) ||
			...
			if (hisifd->panel_info.cascadeic_support) {
			...
			if (hisifd->panel_info.cascadeic_support) {
				if ((pov_req->ovl_idx == DSS_OVL0) || (pov_req->ovl_idx == DSS_OVL1))
					aif1_bw->axi_sel = (hisifd->use_mmbuf_cnt % 2) ? AXI_CHN0 : AXI_CHN1;
				else
					aif1_bw->axi_sel = (hisifd->use_mmbuf_cnt % 2) ? AXI_CHN1 : AXI_CHN0;
			} else {
				if ((pov_req->ovl_idx == DSS_OVL0) || (pov_req->ovl_idx == DSS_OVL1))
		...
		if ((pov_req->ovl_idx == DSS_OVL0) || (pov_req->ovl_idx == DSS_OVL1)) {
			if (tmp <= (bw_sum / 2)) {
				...
				if (axi0_cnt >= AXI0_MAX_DSS_CHN_THRESHOLD) {
					aif_bw_tmp[i - AXI0_MAX_DSS_CHN_THRESHOLD].axi_sel = DSS_ONLINE_AXI1;
					axi1_cnt++;
					axi0_cnt--;
				}
				axi0_cnt++;
			...
			if (tmp <= (bw_sum / 2)) {
				...
				if (axi1_cnt >= AXI1_MAX_DSS_CHN_THRESHOLD) {
					aif_bw_tmp[i - AXI1_MAX_DSS_CHN_THRESHOLD].axi_sel = DSS_OFFLINE_AXI0;
					axi0_cnt++;
					axi1_cnt--;
				}
				axi1_cnt++;
	...
	dss_wb_layer_t *wb_layer, int ovl_idx)
{
	...
	aif_bw = &(hisifd->dss_module.aif_bw[chn_idx]);
	if (aif_bw->is_used != 1) {
		HISI_FB_ERR("fb%d, aif_bw->is_used %d is invalid!", hisifd->index,
			aif_bw->is_used);
		return -EINVAL;
	}
...
int hisi_dss_aif1_ch_config(struct hisi_fb_data_type *hisifd,
	dss_overlay_t *pov_req, dss_layer_t *layer, dss_wb_layer_t *wb_layer,
	int ovl_idx)
{
	dss_aif_t *aif1 = NULL;
	dss_aif_bw_t *aif1_bw = NULL;
	...
	dss_layer_t *layer, dss_wb_layer_t *wb_layer)
{
	...
	for (i = 0; i < g_dss_chn_sid_num[chn_idx]; i++) {
		...
		if (img->mmu_enable == 0) {
			smmu->smmu_smrx_ns[idx] = set_bits32(smmu->smmu_smrx_ns[idx],
				0x1, 1, 0);
		} else {
			if (hisifd->video_idle_ctrl.idle_frame_display) {
...
/*lint +e568 +e685 +e679 +e613 +e838*/
void hisifb_adjust_block_rect(int block_num,
	dss_rect_t *ov_block_rects[], dss_wb_layer_t *wb_layer)
{
	;
}
	...
	dss_layer_t *layer, dss_wb_layer_t *wb_layer)
{
	...
	csc_coe_yuv2rgb = g_csc_coe_yuv2rgb601_narrow;
	csc_coe_rgb2yuv = g_csc_coe_rgb2yuv601_narrow;
	if (csc_mode == DSS_CSC_601_WIDE) {
		csc_coe_yuv2rgb = g_csc_coe_yuv2rgb601_wide;
		csc_coe_rgb2yuv = g_csc_coe_rgb2yuv601_wide;
	} else if (csc_mode == DSS_CSC_601_NARROW) {
		csc_coe_yuv2rgb = g_csc_coe_yuv2rgb601_narrow;
		csc_coe_rgb2yuv = g_csc_coe_rgb2yuv601_narrow;
	} else if (csc_mode == DSS_CSC_709_WIDE) {
		csc_coe_yuv2rgb = g_csc_coe_yuv2rgb709_wide;
		csc_coe_rgb2yuv = g_csc_coe_rgb2yuv709_wide;
	} else if (csc_mode == DSS_CSC_709_NARROW) {
		csc_coe_yuv2rgb = g_csc_coe_yuv2rgb709_narrow;
		csc_coe_rgb2yuv = g_csc_coe_rgb2yuv709_narrow;
	} else if (csc_mode == DSS_CSC_2020) {
		csc_coe_yuv2rgb = g_csc_coe_yuv2rgb2020;
		csc_coe_rgb2yuv = g_csc_coe_rgb2yuv2020;
	}
	...
	if (layer != NULL && hisifd->dss_module.csc_used[chn_idx]) {
		...
		csc->idc0 = set_bits32(csc->idc0, (uint32_t)csc_coe_yuv2rgb[2][3] |
			((uint32_t)csc_coe_yuv2rgb[1][3] << 16), 27, 0);
		csc->idc2 = set_bits32(csc->idc2, csc_coe_yuv2rgb[0][3], 11, 0);

		csc->odc0 = set_bits32(csc->odc0, (uint32_t)csc_coe_yuv2rgb[2][4] |
			((uint32_t)csc_coe_yuv2rgb[1][4] << 16), 27, 0);
	...
	if (layer != NULL && hisifd->dss_module.pcsc_used[chn_idx]) {
		...
		csc->idc0 = set_bits32(csc->idc0, (uint32_t)csc_coe_rgb2yuv[2][3] |
			((uint32_t)csc_coe_rgb2yuv[1][3] << 16), 27, 0);
		csc->idc2 = set_bits32(csc->idc2, csc_coe_rgb2yuv[0][3], 11, 0);

		csc->odc0 = set_bits32(csc->odc0, (uint32_t)csc_coe_rgb2yuv[2][4] |
			((uint32_t)csc_coe_rgb2yuv[1][4] << 16), 27, 0);
	...
	char __iomem *ovl_base, dss_ovl_t *s_ovl, int ovl_idx)
{
	...
	if (ovl_idx == DSS_OVL3) {
		...
		hisifd->set_reg(hisifd, ovl_base + OV2_BLOCK_SIZE,
		...
		hisifd->set_reg(hisifd, ovl_base + OV8_BLOCK_SIZE,
			s_ovl->ovl_block_size, 32, 0);
	}
}
/*lint +e679 +e730 +e732*/
	...
	const int **p, int row, int col)
{
	...
	if ((row != ARSR2P_PHASE_NUM) || ((col != ARSR2P_TAP4) &&
		(col != ARSR2P_TAP6))) {
		HISI_FB_ERR("arsr1p filter coefficients is err, arsr1p_phase_num = %d, "
			"arsr1p_tap_num = %d\n", row, col);
		return -EINVAL;
	}
	/*lint -e679 -e732 -e838*/
...
static void hisi_dss_arsr_post_write_config_coefs(
	struct hisi_fb_data_type *hisifd, char __iomem *addr, const int **scl_down,
...
void hisi_dss_arsr_post_coef_on(struct hisi_fb_data_type *hisifd)
{
	...
	/* COEFY_V COEFY_H */
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefy_v,
		(const int **)g_coef_y_scl_down_tap6, (const int **)g_coef_y_scl_up_tap6,
		ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefy_v + ARSR2P_COEF_H0_OFFSET,
		(const int **)g_coef_y_scl_down_tap6, (const int **)g_coef_y_scl_up_tap6,
		ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefy_v + ARSR2P_COEF_H1_OFFSET,
		(const int **)g_coef_y_scl_down_tap6, (const int **)g_coef_y_scl_up_tap6,
		ARSR2P_PHASE_NUM, ARSR2P_TAP6);

	/* COEFA_V COEFA_H */
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefa_v,
		(const int **)g_coef_auv_scl_down_tap4,
		(const int **)g_coef_auv_scl_up_tap4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefa_v + ARSR2P_COEF_H0_OFFSET,
		(const int **)g_coef_auv_scl_down_tap4, (const int **)g_coef_auv_scl_up_tap4,
		ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefa_v + ARSR2P_COEF_H1_OFFSET,
		(const int **)g_coef_auv_scl_down_tap4, (const int **)g_coef_auv_scl_up_tap4,
		ARSR2P_PHASE_NUM, ARSR2P_TAP4);

	/* COEFUV_V COEFUV_H */
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefuv_v,
		(const int **)g_coef_auv_scl_down_tap4, (const int **)g_coef_auv_scl_up_tap4,
		ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefuv_v + ARSR2P_COEF_H0_OFFSET,
		(const int **)g_coef_auv_scl_down_tap4, (const int **)g_coef_auv_scl_up_tap4,
		ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr_post_write_config_coefs(hisifd, coefuv_v + ARSR2P_COEF_H1_OFFSET,
		(const int **)g_coef_auv_scl_down_tap4, (const int **)g_coef_auv_scl_up_tap4,
		ARSR2P_PHASE_NUM, ARSR2P_TAP4);
}

int hisi_dss_arsr2p_write_coefs(struct hisi_fb_data_type *hisifd,
	bool enable_cmdlist, char __iomem *addr, const int **p, int row, int col)
{
	int coef_value = 0;
	int coef_num = 0;
	int i = 0;
	...
	if ((row != ARSR2P_PHASE_NUM) || ((col != ARSR2P_TAP4) &&
		(col != ARSR2P_TAP6))) {
		HISI_FB_ERR("arsr2p filter coefficients is err, arsr2p_phase_num = %d, "
			"arsr2p_tap_num = %d\n", row, col);
		return -EINVAL;
	}
	/*lint -e679 -e732 -e838*/
...
/*lint -e838*/
void hisi_dss_arsr2p_write_config_coefs(
	struct hisi_fb_data_type *hisifd, bool enable_cmdlist, char __iomem *addr,
	const int **scl_down, const int **scl_up, int row, int col)
{
	int ret = 0;

	ret = hisi_dss_arsr2p_write_coefs(hisifd, enable_cmdlist, addr,
		scl_down, row, col);
	if (ret < 0) {
	...
	ret = hisi_dss_arsr2p_write_coefs(hisifd, enable_cmdlist,
		addr + ARSR2P_SCL_UP_OFFSET, scl_up, row, col);
	if (ret < 0) {
...
/*lint -e570 -e732*/
void hisi_dss_arsr2p_init(const char __iomem *arsr2p_base,
	...
	bool enable_cmdlist)
{
	...
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist,
		coefy_v + ARSR2P_COEF_H0_OFFSET, (const int **)g_coef_y_scl_down_tap6,
		(const int **)g_coef_y_scl_up_tap6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist,
		coefy_v + ARSR2P_COEF_H1_OFFSET, (const int **)g_coef_y_scl_down_tap6,
		(const int **)g_coef_y_scl_up_tap6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
	...
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist, coefa_v,
		...
		ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist,
		coefa_v + ARSR2P_COEF_H0_OFFSET, (const int **)g_coef_auv_scl_down_tap4,
		(const int **)g_coef_auv_scl_up_tap4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist,
		coefa_v + ARSR2P_COEF_H1_OFFSET, (const int **)g_coef_auv_scl_down_tap4,
		(const int **)g_coef_auv_scl_up_tap4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	...
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist, coefuv_v,
		...
		ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist,
		coefuv_v + ARSR2P_COEF_H0_OFFSET, (const int **)g_coef_auv_scl_down_tap4,
		(const int **)g_coef_auv_scl_up_tap4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
	hisi_dss_arsr2p_write_config_coefs(hisifd, enable_cmdlist,
		coefuv_v + ARSR2P_COEF_H1_OFFSET, (const int **)g_coef_auv_scl_down_tap4,
		(const int **)g_coef_auv_scl_up_tap4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
}
/*lint +e838*/
static int hisi_dss_arsr2p_config_check_width(
	dss_rect_t *dest_rect, int source_width, bool hscl_en, bool vscl_en,
	...
	if ((source_width < ARSR2P_MIN_INPUT) || (dest_rect->w < ARSR2P_MIN_INPUT) ||
		...
		if ((!hscl_en) && (!vscl_en)) {
			...
			HISI_FB_DEBUG("fb%d, src_w %d is not between [%d, %d]\n"
				"or dst_w is not between [%d, %d], but arsr2p bypass!\n",
				index, source_width, ARSR2P_MIN_INPUT, ARSR2P_MAX_WIDTH,
				dest_rect->w, ARSR2P_MIN_INPUT, ARSR2P_DST_MAX_WIDTH);
			return 0;
		}

		HISI_FB_ERR("fb%d, src_w %d is not between [%d, %d]\n"
			"or dst_w is not between [%d, %d]!\n",
			index, source_width, ARSR2P_MIN_INPUT, ARSR2P_MAX_WIDTH,
			dest_rect->w, ARSR2P_MIN_INPUT, ARSR2P_DST_MAX_WIDTH);
		return -EINVAL;
	}
	...
	dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	do {
		...
		if (pblock_info != NULL && pblock_info->h_ratio_arsr2p) {
			...
			ih_inc = (DSS_WIDTH(src_width) * ARSR2P_INC_FACTOR +
		...
		outph_right = (dst_rect.x + dst_rect.w - 1) * ih_inc -
		...
		if (dst_whole_width == dst_rect.w) {
			...
			extraw = (dst_whole_width - 1) * ih_inc -
	...
	do {
		...
		if (src_rect.h > dst_rect.h) {
			...
			iv_inc = (DSS_HEIGHT(src_rect.h) * ARSR2P_INC_FACTOR +
				ARSR2P_INC_FACTOR / 2 - iv_top) / DSS_HEIGHT(dst_rect.h);
		} else {
	...
	ret = hisi_dss_arsr2p_config_check_width(&dst_rect, src_width, en_hscl,
		en_vscl, src_height, hisifd->index);
	if (ret <= 0)
		return ret;
	...
	do {
		...
		if (!en_hscl && (iv_inc >= 2 * ARSR2P_INC_FACTOR) &&
			...
			// only vertical scale down, enable nearest scaling down,
			// disable sharp in non-block scene
			nearest_en = true;
			break;
		}
	...
	arsr2p->arsr_input_width_height = set_bits32(
		arsr2p->arsr_input_width_height, DSS_WIDTH(src_width), 13, 16);
	...
	dss_wb_layer_t *wb_layer)
{
	...
	if (pblock_info && (pblock_info->src_rect.w != 0) &&
		(pblock_info->dst_rect.w != 0)) {
	...
	do {
		...
		h_ratio = (DSS_HEIGHT(src_rect.w) * SCF_INC_FACTOR +
	...
	do {
		...
		v_ratio = (DSS_HEIGHT(src_rect.h) * SCF_INC_FACTOR +
...
{
	if (hisifd == NULL || post_clip_base == NULL || s_post_clip == NULL) {
	...
	struct hisi_fb_data_type *hisifd, dss_wb_layer_t *wb_layer)
{
	...
	if (wb_layer->dst_rect.x < 0 || wb_layer->dst_rect.y < 0 ||
		...
		wb_layer->dst_rect.w > wb_layer->dst.width ||
		wb_layer->dst_rect.h > wb_layer->dst.height) {
	...
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block_infos)
{
	...
	if (hisifd->index == AUXILIARY_PANEL_IDX) {
		...
		for (i = 0; i < pov_req->wb_layer_nums; i++) {
			...
			if ((wb_layer->dst.bpp == 0) || (wb_layer->dst.width == 0) ||
	...
	dss_layer_t *layer, uint32_t index)
{
	...
	if (layer->need_cap & CAP_AFBCD) {
		if ((layer->img.afbc_header_stride == 0) ||
			...
			HISI_FB_ERR("fb%d, afbc_header_stride=%d, afbc_payload_stride=%d, "
	...
	dss_layer_t *layer)
{
	...
	if (layer->blending < 0 || layer->blending >= HISI_FB_BLENDING_MAX) {
		HISI_FB_ERR("fb%d, blending=%d is invalid!",
...
static int hisi_dss_hfbcd_mmbuf_config(
	...
	dss_layer_t *layer, uint32_t hfbcd_block_type, bool is_pixel_10bit)
{
	...
	if (mm_alloc_needed) {
		...
		hfbcd_rect.right = ALIGN_UP(
			(uint32_t)(src_rect.x - hfbcd_rect.left + src_rect.w),
			MMBUF_ADDR_ALIGN);

		if (hfbcd_block_type == 0) {
			if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
				mmbuf_line_num = MMBUF_BLOCK0_ROT_LINE_NUM;
				hfbcd_rect.left = ALIGN_DOWN((uint32_t)src_rect.y,
					MMBUF_ADDR_ALIGN);
				hfbcd_rect.right = ALIGN_UP(
					(uint32_t)(src_rect.y - hfbcd_rect.left + src_rect.h),
					MMBUF_ADDR_ALIGN);
			} else {
		...
		hisifd->mmbuf_info->mm_size0_y8[chn_idx] =
			hfbcd_rect.right * mmbuf_line_num;
		hisifd->mmbuf_info->mm_size1_c8[chn_idx] =
			hisifd->mmbuf_info->mm_size0_y8[chn_idx] / 2;
		if (is_pixel_10bit) {
			if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
				hfbcd_rect.left = ALIGN_DOWN(
					(uint32_t)src_rect.y, MMBUF_ADDR_ALIGN);
				...
				hfbcd_rect.left = ALIGN_DOWN(
					(uint32_t)src_rect.x, MMBUF_ADDR_ALIGN);
				hfbcd_rect.right = ALIGN_UP(ALIGN_UP(
					(uint32_t)(src_rect.x - hfbcd_rect.left + src_rect.w),
					MMBUF_ADDR_ALIGN) / 4,
					MMBUF_ADDR_ALIGN);
			...
			hisifd->mmbuf_info->mm_size3_c2[chn_idx] =
				hisifd->mmbuf_info->mm_size2_y2[chn_idx] / 2;
		}
		...
		hisifd->mmbuf_info->mm_base[chn_idx] =
			...
			hisifd->mmbuf_info->mm_size[chn_idx]);
		hisifd->mmbuf_info->mm_base0_y8[chn_idx] =
			hisifd->mmbuf_info->mm_base[chn_idx];
		hisifd->mmbuf_info->mm_base1_c8[chn_idx] =
			hisifd->mmbuf_info->mm_base0_y8[chn_idx] +
			hisifd->mmbuf_info->mm_size0_y8[chn_idx];
		hisifd->mmbuf_info->mm_base2_y2[chn_idx] =
			hisifd->mmbuf_info->mm_base1_c8[chn_idx] +
			hisifd->mmbuf_info->mm_size1_c8[chn_idx];
		hisifd->mmbuf_info->mm_base3_c2[chn_idx] =
			hisifd->mmbuf_info->mm_base2_y2[chn_idx] +
			hisifd->mmbuf_info->mm_size2_y2[chn_idx];
		...
		if (is_pixel_10bit) {
			if ((hisifd->mmbuf_info->mm_base2_y2[chn_idx] < MMBUF_BASE) ||
				...
				HISI_FB_ERR("fb%d, chn%d failed to alloc mmbuf, "
					"mm_base2_y2=0x%x, mm_base3_c2=0x%x.\n",
					hisifd->index, chn_idx,
					hisifd->mmbuf_info->mm_base2_y2[chn_idx],
					hisifd->mmbuf_info->mm_base3_c2[chn_idx]);
				return -EINVAL;
...
int hisi_dss_hfbcd_config(struct hisi_fb_data_type *hisifd,
	int ovl_idx, dss_layer_t *layer, dss_rect_ltrb_t *clip_rect,
	dss_rect_t *out_aligned_rect, bool *rdma_stretch_enable)
{
	...
	uint32_t hfbcd_header_offset = 0;
	...
	*rdma_stretch_enable = (stretched_line_num > 0) ? true : false;
	...
	if ((layer->img.hfbc_mmbuf_base0_y8 > 0) &&
		...
		ret = hisi_dss_hfbcd_mmbuf_config(hisifd, ovl_idx, layer,
			hfbcd_block_type, is_pixel_10bit);
		if (ret < 0) {
	...
	ret = hisi_dss_hfbcd_config_check_mm_base0(hisifd, mm_base0_y8,
		mm_base1_c8, chn_idx, layer->layer_idx);
	if (ret == -EINVAL)
		return ret;
	...
	if (is_pixel_10bit) {
		...
		ret = hisi_dss_hfbcd_config_check_mm_base2(hisifd, mm_base2_y2,
			mm_base3_c2, chn_idx, layer->layer_idx);
		if (ret == -EINVAL)
			return ret;
	}
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		if ((hfbcd_hreg_pic_width > HFBC_PIC_WIDTH_ROT_MAX) ||
			(hfbcd_hreg_pic_height > HFBC_PIC_HEIGHT_ROT_MAX)) {
			HISI_FB_ERR("layer%d hfbcd_hreg_pic_width %d, "
				"hfbcd_hreg_pic_height %d is too large 4096*2160!\n",
	...
	hfbcd_header_offset =
		(aligned_rect.top / hfbcd_block_height_align) * hfbcd_header_stride0 +
		(aligned_rect.left / hfbcd_block_width_align) * HFBC_HEADER_STRIDE_BLOCK;

	hfbcd_header_addr0 = layer->img.hfbc_header_addr0 + hfbcd_header_offset;
	hfbcd_header_addr1 = layer->img.hfbc_header_addr1 + hfbcd_header_offset;
	hfbcd_header_stride1 = hfbcd_header_stride0;
	if ((hfbcd_header_addr0 & (HFBC_HEADER_ADDR_ALIGN - 1)) ||
		(hfbcd_header_stride0 & (HFBC_HEADER_STRIDE_ALIGN - 1)) ||
		(hfbcd_header_addr1 & (HFBC_HEADER_ADDR_ALIGN - 1))) {
		HISI_FB_ERR("layer%d hfbcd_header_addr0 0x%x or "
			"hfbcd_header_addr1 0x%x is not %d bytes aligned or "
			"hfbcd_header_stride0 0x%x is not %d bytes aligned!\n",
			layer->layer_idx, hfbcd_header_addr0, hfbcd_header_addr1,
			HFBC_HEADER_ADDR_ALIGN, hfbcd_header_stride0,
			HFBC_HEADER_STRIDE_ALIGN);
		return -EINVAL;
	}
	...
	if (is_pixel_10bit) {
		if ((hfbcd_payload_addr0 != hfbcd_payload_addr1) ||
	...
	if ((hfbcd_payload_addr0 & (hfbcd_payload0_align - 1)) ||
		(hfbcd_payload_stride0 & (hfbcd_payload0_align - 1)) ||
		(hfbcd_payload_addr1 & (hfbcd_payload1_align - 1)) ||
		(hfbcd_payload_stride1 & (hfbcd_payload1_align - 1))) {
		HISI_FB_ERR("layer%d hfbc_payload_addr0 0x%x or "
			"hfbc_payload_stride0 0x%x is not %d bytes aligned, or "
			"hfbc_payload_addr1 0x%x or hfbc_payload_stride1 0x%x is "
			"not %d bytes aligned!\n",
			layer->layer_idx, hfbcd_payload_addr0, hfbcd_payload_stride0,
			hfbcd_payload0_align, hfbcd_payload_addr1, hfbcd_payload_stride1,
			hfbcd_payload1_align);
		return -EINVAL;
	}

	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		aligned_rect.left = ALIGN_DOWN(
			(uint32_t)src_rect.y, hfbcd_block_height_align);
	...
	if (hisi_adjust_clip_rect(layer, clip_rect) < 0) {
		HISI_FB_ERR("clip rect invalid => layer_idx=%d, chn_idx=%d, "
			"clip_rect %d, %d, %d, %d.\n",
	...
	/*lint -e574*/
	// hfbcd crop
	hfbcd_top_crop_num = (clip_rect->top > hfbcd_crop_num_max) ?
		hfbcd_crop_num_max : clip_rect->top;
	hfbcd_bottom_crop_num = (clip_rect->bottom > hfbcd_crop_num_max) ?
		hfbcd_crop_num_max : clip_rect->bottom;
	/*lint +e574*/
	...
	dma->hfbcd_input_header_stride0 = set_bits32(
		dma->hfbcd_input_header_stride0, hfbcd_header_stride0, 14, 0);
	dma->hfbcd_hreg_hdr_ptr_l1 = set_bits32(
		dma->hfbcd_hreg_hdr_ptr_l1, hfbcd_header_addr1, 32, 0);
	dma->hfbcd_header_stride1 = set_bits32(
		dma->hfbcd_header_stride1, hfbcd_header_stride1, 14, 0);
	dma->hfbcd_mm_base0_y8 = set_bits32(
		dma->hfbcd_mm_base0_y8, mm_base0_y8, 32, 0);
	dma->hfbcd_mm_base1_c8 = set_bits32(
		dma->hfbcd_mm_base1_c8, mm_base1_c8, 32, 0);
	dma->hfbcd_mm_base2_y2 = set_bits32(
		dma->hfbcd_mm_base2_y2, mm_base2_y2, 32, 0);
	dma->hfbcd_mm_base3_c2 = set_bits32(
		dma->hfbcd_mm_base3_c2, mm_base3_c2, 32, 0);
	...
	dma->hfbcd_creg_fbcd_ctrl_mode = set_bits32(
		dma->hfbcd_creg_fbcd_ctrl_mode, 1, 2, 0);
	// hfbcd_scramble_mode
	dma->hfbcd_scramble_mode = set_bits32(dma->hfbcd_scramble_mode,
		layer->img.hfbc_scramble_mode, 4, 2);
	dma->hfbcd_block_type = set_bits32(
		dma->hfbcd_block_type, hfbcd_block_type, 2, 0);
	...
	if (g_debug_ovl_online_composer || g_debug_ovl_offline_composer)
		HISI_FB_INFO("fb%d, mm_base0_y8=0x%x, mm_base2_y2=0x%x, "
			...
			hisifd->index, mm_base0_y8, mm_base2_y2, mm_base1_c8,
			mm_base3_c2, layer->img.hfbc_mmbuf_size0_y8,
			layer->img.hfbc_mmbuf_size2_y2, layer->img.hfbc_mmbuf_size1_c8,
			layer->img.hfbc_mmbuf_size3_c2,
			aligned_rect.left, aligned_rect.top,
			aligned_rect.right, aligned_rect.bottom,
			hfbcd_block_type);
...
int hisi_dss_hfbce_config(struct hisi_fb_data_type *hisifd,
	dss_overlay_t *pov_req, dss_wb_layer_t *layer, dss_rect_t aligned_rect,
	dss_rect_t *ov_block_rect, bool last_block)
{
	...
	uint32_t hfbce_header_offset = 0;
	...
	if ((layer->dst.width & (hfbce_block_width_align - 1)) ||
		...
		HISI_FB_ERR("wb_layer dst width %d is not %d bytes aligned, "
			...
			layer->dst.width, hfbce_block_width_align,
			layer->dst.height, hfbce_block_height_align);
		return -EINVAL;
	}
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		hfbce_header_rect.left = ALIGN_DOWN(
			(uint32_t)layer->dst_rect.x, hfbce_block_width_align);
		hfbce_header_rect.top = ALIGN_DOWN(
			(uint32_t)(layer->dst_rect.y + (ov_block_rect->x - layer->dst_rect.x)),
			hfbce_block_height_align);

		hfbce_payload_rect.left = ALIGN_DOWN(
			(uint32_t)layer->dst_rect.x, hfbce_block_width_align);
		hfbce_payload_rect.top = hfbce_header_rect.top;

		hfbce_hreg_pic_blks = (in_rect.h / hfbce_block_width_align) *
			(in_rect.w / hfbce_block_height_align) - 1;
	} else {
		hfbce_header_rect.left = ALIGN_DOWN(
			(uint32_t)in_rect.x, hfbce_block_width_align);
		hfbce_header_rect.top = ALIGN_DOWN(
			(uint32_t)in_rect.y, hfbce_block_height_align);

		hfbce_payload_rect.left = ALIGN_DOWN(
			(uint32_t)in_rect.x, hfbce_block_width_align);
		hfbce_payload_rect.top = hfbce_header_rect.top;

		hfbce_hreg_pic_blks = (in_rect.w / hfbce_block_width_align) *
			(in_rect.h / hfbce_block_height_align) - 1;
	}
	// hfbc header
	hfbce_header_stride0 = layer->dst.hfbc_header_stride0;
	hfbce_header_offset = (hfbce_header_rect.top /
		hfbce_block_height_align) * hfbce_header_stride0 +
		(hfbce_header_rect.left / hfbce_block_width_align) *
		HFBC_HEADER_STRIDE_BLOCK;

	hfbce_header_addr0 = layer->dst.hfbc_header_addr0 + hfbce_header_offset;
	hfbce_header_addr1 = layer->dst.hfbc_header_addr1 + hfbce_header_offset;
	hfbce_header_stride1 = hfbce_header_stride0;
	if ((hfbce_header_addr0 & (HFBC_HEADER_ADDR_ALIGN - 1)) ||
		(hfbce_header_stride0 & (HFBC_HEADER_STRIDE_ALIGN - 1)) ||
		(hfbce_header_addr1 & (HFBC_HEADER_ADDR_ALIGN - 1))) {
		HISI_FB_ERR("wb_layer hfbce_header_addr0 0x%x or "
			"hfbcd_header_addr1 0x%x is not %d bytes aligned, or "
	...
	hfbce_payload_addr0 = layer->dst.hfbc_payload_addr0 +
		(hfbce_payload_rect.top / hfbce_block_height_align) *
		hfbce_payload_stride0 +
		(hfbce_payload_rect.left / hfbce_block_width_align) *
		hfbce_payload0_align;

	hfbce_payload_addr1 = layer->dst.hfbc_payload_addr1 +
		(hfbce_payload_rect.top / hfbce_block_height_align) *
		hfbce_payload_stride1 +
		(hfbce_payload_rect.left / hfbce_block_width_align) *
		hfbce_payload1_align;

	if (is_pixel_10bit) {
		if ((hfbce_payload_addr0 != hfbce_payload_addr1) ||
	...
	if ((hfbce_payload_addr0 & (hfbce_payload0_align - 1)) ||
		(hfbce_payload_stride0 & (hfbce_payload0_align - 1)) ||
		(hfbce_payload_addr1 & (hfbce_payload1_align - 1)) ||
		(hfbce_payload_stride1 & (hfbce_payload1_align - 1))) {
		HISI_FB_ERR("ch%d hfbce_payload_addr0 0x%x or "
			"hfbce_payload_stride0 0x%x is not %d bytes aligned, or "
			"hfbce_payload_addr1 0x%x or hfbce_payload_stride1 0x%x is "
			"not %d bytes aligned!\n",
			layer->chn_idx, hfbce_payload_addr0, hfbce_payload_stride0,
			hfbce_payload0_align, hfbce_payload_addr1, hfbce_payload_stride1,
			hfbce_payload1_align);
		return -EINVAL;
	}
	...
	wdma->hfbce_hreg_pic_blks = set_bits32(
		wdma->hfbce_hreg_pic_blks, hfbce_hreg_pic_blks, 24, 0);
	wdma->hfbce_hreg_hdr_ptr_l0 = set_bits32(
		wdma->hfbce_hreg_hdr_ptr_l0, hfbce_header_addr0, 32, 0);
	wdma->hfbce_hreg_pld_ptr_l0 = set_bits32(
		wdma->hfbce_hreg_pld_ptr_l0, hfbce_payload_addr0, 32, 0);
	...
	wdma->hfbce_header_stride0 = set_bits32(
		wdma->hfbce_header_stride0, hfbce_header_stride0, 14, 0);
	wdma->hfbce_payload_stride0 = set_bits32(
		wdma->hfbce_payload_stride0, hfbce_payload_stride0, 20, 0);
	wdma->hfbce_scramble_mode = set_bits32(
		wdma->hfbce_scramble_mode,
		layer->dst.hfbc_scramble_mode, 4, 2);
	wdma->hfbce_header_pointer_offset = set_bits32(
		wdma->hfbce_header_pointer_offset, hfbce_header_offset, 32, 0);
	wdma->fbce_creg_fbce_ctrl_mode = set_bits32(
		wdma->fbce_creg_fbce_ctrl_mode, 1, 32, 0);
	wdma->hfbce_hreg_hdr_ptr_l1 = set_bits32(
		wdma->hfbce_hreg_hdr_ptr_l1, hfbce_header_addr1, 32, 0);
	wdma->hfbce_hreg_pld_ptr_l1 = set_bits32(
		wdma->hfbce_hreg_pld_ptr_l1, hfbce_payload_addr1, 32, 0);
	wdma->hfbce_header_stride1 = set_bits32(
		wdma->hfbce_header_stride1, hfbce_header_stride1, 14, 0);
	wdma->hfbce_payload_stride1 = set_bits32(
		wdma->hfbce_payload_stride1, hfbce_payload_stride1, 20, 0);
	...
	bool is_yuv_semi_planar)
{
	...
	if (mm_alloc_needed) {
		...
		hisifd->mmbuf_info->mm_size0_y8[chn_idx] =
			hebcd_rect.right * mmbuf_y8_line_num;
		if (is_yuv_semi_planar)
			hisifd->mmbuf_info->mm_size1_c8[chn_idx] =
				hebcd_rect.right * mmbuf_c8_line_num;
		else
...
int hisi_dss_hebcd_config(struct hisi_fb_data_type *hisifd,
	int ovl_idx, dss_layer_t *layer, dss_rect_ltrb_t *clip_rect,
	dss_rect_t *out_aligned_rect, bool *rdma_stretch_enable)
{
	dss_rdma_t *dma = NULL;
	...
	uint32_t hebcd_header_offset = 0;
	...
	*rdma_stretch_enable = (stretched_line_num > 0) ? true : false;
	...
	if (layer->img.hebc_mmbuf_base0_y8 > 0) {
		...
		ret = hisi_dss_hebcd_mmbuf_config(hisifd, ovl_idx, layer,
			hebcd_block_type, is_yuv_semi_planar);
		if (ret < 0) {
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		if ((hebcd_hreg_pic_width > HFBC_PIC_WIDTH_ROT_MAX) ||
			...
			HISI_FB_ERR("layer%d hfbcd_hreg_pic_width %d, hfbcd_hreg_pic_height %d "
				...
				hebcd_hreg_pic_width, hebcd_hreg_pic_height);
			return -EINVAL;
		}
	}
	...
	hebcd_header_offset = (aligned_rect.top / hebcd_block_height_align) *
		hebcd_header_stride0 + (aligned_rect.left / hebcd_block_width_align) *
		HEBC_HEADER_STRIDE_BLOCK;

	hebcd_header_addr0 = layer->img.hebc_header_addr0 + hebcd_header_offset;
	...
	if (is_yuv_semi_planar) {
		hebcd_header_addr1 = layer->img.hebc_header_addr1 + hebcd_header_offset;
	...
	if (is_yuv_semi_planar) {
		...
		hebcd_payload_addr1 = layer->img.hebc_payload_addr1 +
			(aligned_rect.top / hebcd_block_height_align) *
			hebcd_payload_stride1 +
			(aligned_rect.left / hebcd_block_width_align) *
			hebcd_payload1_align;
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		aligned_rect.left = ALIGN_DOWN(
			(uint32_t)src_rect.y, hebcd_block_height_align);
		aligned_rect.right = ALIGN_UP(src_bottom, hebcd_block_height_align) - 1; //lint !e502
		aligned_rect.top = ALIGN_DOWN(
			(uint32_t)src_rect.x, hebcd_block_width_align);
	...
	hebcd_top_crop_num = (clip_rect->top > hebcd_crop_num_max) ?
		hebcd_crop_num_max : clip_rect->top;
	hebcd_bottom_crop_num = (clip_rect->bottom > hebcd_crop_num_max) ?
		hebcd_crop_num_max : clip_rect->bottom;
	...
	dma->hebcd_hreg_hdr_ptr_l0 = set_bits32(
		dma->hebcd_hreg_hdr_ptr_l0, hebcd_header_addr0, 32, 0);
	dma->hebcd_hreg_pic_width = set_bits32(
		dma->hebcd_hreg_pic_width, hebcd_hreg_pic_width, 16, 0);
	dma->hebcd_hreg_pic_height = set_bits32(
		dma->hebcd_hreg_pic_height, hebcd_hreg_pic_height, 16, 0);
	dma->hebcd_hreg_format = set_bits32(
		dma->hebcd_hreg_format, color_transform, 1, 21);
	dma->hebcd_line_crop = set_bits32(
		dma->hebcd_line_crop,
		(hebcd_top_crop_num << 8 | hebcd_bottom_crop_num), 16, 0);
	dma->hebcd_input_header_stride0 = set_bits32(
		dma->hebcd_input_header_stride0, hebcd_header_stride0, 14, 0);
	dma->hebcd_payload_stride0 = set_bits32(
		dma->hebcd_payload_stride0, hebcd_payload_stride0, 20, 0);
	dma->hebcd_mm_base0_y8 = set_bits32(
		dma->hebcd_mm_base0_y8, mm_base0_y8, 32, 0);
	dma->hebcd_payload_pointer = set_bits32(
		dma->hebcd_payload_pointer, hebcd_payload_addr0, 32, 0);
	dma->hebcd_height_bf_str = set_bits32(
		dma->hebcd_height_bf_str, DSS_HEIGHT(hebcd_height_bf_str), 16, 0);
	dma->hebcd_scramble_mode = set_bits32(
		dma->hebcd_scramble_mode, layer->img.hebc_scramble_mode, 4, 8);
	dma->hebcd_creg_fbcd_ctrl_mode = set_bits32(
		dma->hebcd_creg_fbcd_ctrl_mode, 0x10, 32, 0);
	dma->hebcd_hreg_hdr_ptr_l1 = set_bits32(
		dma->hebcd_hreg_hdr_ptr_l1, hebcd_header_addr1, 32, 0);
	dma->hebcd_hreg_pld_ptr_l1 = set_bits32(
		dma->hebcd_hreg_pld_ptr_l1, hebcd_payload_addr1, 32, 0);
	dma->hebcd_header_stride1 = set_bits32(
		dma->hebcd_header_stride1, hebcd_header_stride1, 14, 0);
	dma->hebcd_payload_stride1 = set_bits32(
		dma->hebcd_payload_stride1, hebcd_payload_stride1, 20, 0);
	dma->hebcd_block_type = set_bits32(
		dma->hebcd_block_type, hebcd_block_type, 2, 0);
	dma->hebcd_mm_base1_c8 = set_bits32(
		dma->hebcd_mm_base1_c8, mm_base1_c8, 32, 0);
	...
	if (g_debug_ovl_online_composer || g_debug_ovl_offline_composer)
		HISI_FB_INFO("fb%d, mm_base0_y8=0x%x, mm_base1_c8=0x%x,"
			...
			hisifd->index, mm_base0_y8, mm_base1_c8,
			layer->img.hebc_mmbuf_size0_y8,
			layer->img.hebc_mmbuf_size1_c8,
			aligned_rect.left, aligned_rect.top,
			aligned_rect.right, aligned_rect.bottom,
			hebcd_block_type);
	...
	dss_rect_t *ov_block_rect, bool last_block)
{
	...
	uint32_t hebce_header_offset = 0;
	...
	if (hisifd == NULL || pov_req == NULL || layer == NULL ||
		ov_block_rect == NULL)
		return -EINVAL;
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		if (is_yuv_format) {
			if ((in_rect.w < HEBC_YUV_WIDTH_ROT_MIN) ||
				(in_rect.w > HEBC_YUV_WIDTH_ROT_MAX) ||
				(in_rect.h < HEBC_YUV_HEIGHT_ROT_MIN) ||
				(in_rect.h > HEBC_YUV_HEIGHT_ROT_MAX) ||
				(in_rect.w & (HEBC_BLOCK1_YUV_HEIGHT_ALIGN - 1)) ||
				(in_rect.h & (HEBC_BLOCK1_YUV_WIDTH_ALIGN - 1))) {
				HISI_FB_ERR("hebce in_rect %d,%d, %d,%d is out of range!",
					in_rect.x, in_rect.y, in_rect.w, in_rect.h);
				return -EINVAL;
			}
		} else {
			if ((in_rect.w < HEBC_RGB_WIDTH_ROT_MIN) ||
				(in_rect.w > HEBC_RGB_WIDTH_ROT_MAX) ||
				(in_rect.h < HEBC_RGB_HEIGHT_ROT_MIN) ||
				(in_rect.h > HEBC_RGB_HEIGHT_ROT_MAX) ||
				(in_rect.w & (HEBC_BLOCK1_RGB_HEIGHT_ALIGN - 1)) ||
				(in_rect.h & (HEBC_BLOCK1_RGB_WIDTH_ALIGN - 1))) {
				HISI_FB_ERR("hebce in_rect %d,%d, %d,%d is out of range!",
					in_rect.x, in_rect.y, in_rect.w, in_rect.h);
				return -EINVAL;
			}
		...
		if (is_yuv_format) {
			if ((in_rect.w < HEBC_YUV_WIDTH_MIN) ||
				(in_rect.w > HEBC_YUV_WIDTH_MAX) ||
				(in_rect.h < HEBC_YUV_HEIGHT_MIN) ||
				(in_rect.h > HEBC_YUV_HEIGHT_MAX) ||
				(in_rect.w & (HEBC_BLOCK0_YUV_WIDTH_ALIGN - 1)) ||
				(in_rect.h & (HEBC_BLOCK0_YUV_HEIGHT_ALIGN - 1))) {
				HISI_FB_ERR("hebce in_rect %d,%d, %d,%d is out of range!",
					in_rect.x, in_rect.y, in_rect.w, in_rect.h);
				return -EINVAL;
			}
		} else {
			if ((in_rect.w < HEBC_RGB_WIDTH_MIN) ||
				(in_rect.w > HEBC_RGB_WIDTH_MAX) ||
				(in_rect.h < HEBC_RGB_HEIGHT_MIN) ||
				(in_rect.h > HEBC_RGB_HEIGHT_MAX) ||
				(in_rect.w & (HEBC_BLOCK0_RGB_WIDTH_ALIGN - 1)) ||
				(in_rect.h & (HEBC_BLOCK0_RGB_HEIGHT_ALIGN - 1))) {
				HISI_FB_ERR("hebce in_rect %d,%d, %d,%d is out of range!",
					in_rect.x, in_rect.y, in_rect.w, in_rect.h);
				return -EINVAL;
			}
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		hebce_header_rect.left = ALIGN_DOWN(layer->dst_rect.x,
			hebce_block_width_align);
		hebce_header_rect.top = ALIGN_DOWN(layer->dst_rect.y +
			(ov_block_rect->x - layer->dst_rect.x),
			hebce_block_height_align);

		hebce_payload_rect.left = ALIGN_DOWN(layer->dst_rect.x,
			hebce_block_width_align);
		hebce_payload_rect.top = hebce_header_rect.top;
	} else {
		hebce_header_rect.left = ALIGN_DOWN(in_rect.x,
			hebce_block_width_align);
		hebce_header_rect.top = ALIGN_DOWN(in_rect.y,
			hebce_block_height_align);

		hebce_payload_rect.left = ALIGN_DOWN(in_rect.x,
			hebce_block_width_align);
		hebce_payload_rect.top = hebce_header_rect.top;
	}
	...
	if (is_yuv_format) {
		hebce_header_offset =
			(hebce_header_rect.top / hebce_block_height_align) *
			hebce_header_stride0 +
			(hebce_header_rect.left / hebce_block_width_align) *
			HEBC_HEADER_STRIDE_BLOCK;
		hebce_header_addr1 = layer->dst.hebc_header_addr1 + hebce_header_offset;
		...
		hebce_payload_addr1 = layer->dst.hebc_payload_addr1 +
			(hebce_payload_rect.top / hebce_block_height_align) *
			hebce_payload_stride1 +
			(hebce_payload_rect.left / hebce_block_width_align) *
			hebce_payload1_align;

		if ((hebce_payload_addr1 & (hebce_payload1_align - 1)) ||
			(hebce_payload_stride1 & (hebce_payload1_align - 1))) {
			HISI_FB_ERR("ch%d hebce_payload_addr1 0x%x or hebce_payload_stride1"
				" 0x%x is not %d bytes aligned!\n",
				layer->chn_idx, hebce_payload_addr1,
				hebce_payload_stride1, hebce_payload1_align);
			return -EINVAL;
		}
	...
	hebce_header_offset =
		...
		(hebce_header_rect.left / hebce_block_width_align) *
		HEBC_HEADER_STRIDE_BLOCK;
	hebce_header_addr0 = layer->dst.hebc_header_addr0 + hebce_header_offset;
	if ((hebce_header_addr0 & (HEBC_HEADER_ADDR_ALIGN - 1)) ||
		(hebce_header_stride0 & (HEBC_HEADER_STRIDE_ALIGN - 1))) {
		HISI_FB_ERR("wb_layer hebce_header_addr0 0x%x is not %d bytes aligned, or "
			"hebce_header_stride0 0x%x is not %d bytes aligned!\n",
			hebce_header_addr0, HEBC_HEADER_ADDR_ALIGN,
			hebce_header_stride0, HEBC_HEADER_STRIDE_ALIGN);
		return -EINVAL;
	}
	...
	hebce_payload_addr0 = layer->dst.hebc_payload_addr0 +
		(hebce_payload_rect.top / hebce_block_height_align) *
		hebce_payload_stride0 +
		(hebce_payload_rect.left / hebce_block_width_align) *
		hebce_payload0_align;

	if ((hebce_payload_addr0 & (hebce_payload0_align - 1)) ||
		(hebce_payload_stride0 & (hebce_payload0_align - 1))) {
		HISI_FB_ERR("ch%d hebce_payload_addr0 0x%x or hebce_payload0_align"
			" 0x%x is not %d bytes aligned!\n",
			layer->chn_idx, hebce_payload_addr0,
			hebce_payload_stride0, hebce_payload0_align);
		return -EINVAL;
	}
	...
	// color transform
	wdma->hebce_hreg_format = set_bits32(
		wdma->hebce_hreg_format, color_transform, 1, 21);
	wdma->hebce_hreg_hdr_ptr_l0 = set_bits32(
		wdma->hebce_hreg_hdr_ptr_l0, hebce_header_addr0, 32, 0);
	wdma->hebce_hreg_pld_ptr_l0 = set_bits32(
		wdma->hebce_hreg_pld_ptr_l0, hebce_payload_addr0, 32, 0);
	wdma->hebce_picture_size = set_bits32(
		wdma->hebce_picture_size,
		((DSS_WIDTH(in_rect.w) << 16) | DSS_HEIGHT(in_rect.h)), 32, 0);
	wdma->hebce_header_stride0 = set_bits32(
		wdma->hebce_header_stride0, hebce_header_stride0, 14, 0);
	wdma->hebce_payload_stride0 = set_bits32(
		wdma->hebce_payload_stride0, hebce_payload_stride0, 20, 0);
	wdma->hebce_scramble_mode = set_bits32(
		wdma->hebce_scramble_mode, layer->dst.hebc_scramble_mode, 4, 6);
	// HEBCE enable
	wdma->fbce_creg_fbce_ctrl_mode = set_bits32(
		wdma->fbce_creg_fbce_ctrl_mode, 0x20, 32, 0);
	wdma->hebce_hreg_hdr_ptr_l1 = set_bits32(
		wdma->hebce_hreg_hdr_ptr_l1, hebce_header_addr1, 32, 0);
	wdma->hebce_hreg_pld_ptr_l1 = set_bits32(
		wdma->hebce_hreg_pld_ptr_l1, hebce_payload_addr1, 32, 0);
	wdma->hebce_header_stride1 = set_bits32(
		wdma->hebce_header_stride1, hebce_header_stride1, 14, 0);
	wdma->hebce_payload_stride1 = set_bits32(
		wdma->hebce_payload_stride1, hebce_payload_stride1, 20, 0);
	wdma->hebce_spblock_mode = set_bits32(
		wdma->hebce_spblock_mode, 0x1, 32, 0);

	if (g_debug_ovl_offline_composer)
		HISI_FB_INFO("aligned_rect %d,%d,%d,%d, hebce_payload_rect %d,%d,%d,%d,"
			" hebce_header_offset=0x%x!\n",
			in_rect.x, in_rect.y, DSS_WIDTH(in_rect.x + in_rect.w),
			DSS_WIDTH(in_rect.y + in_rect.h),
			hebce_payload_rect.left, hebce_payload_rect.top,
			hebce_payload_rect.right, hebce_payload_rect.bottom,
			hebce_header_offset);
	...
	dss_mctl_sys_t *s_mctl_sys)
{
	...
	for (i = 0; i < DSS_WCH_MAX; i++)
		s_mctl_sys->wchn_ov_sel[i] =
			inp32(mctl_sys_base + MCTL_WCH_OV2_SEL + i * 0x4);
}
	...
	if (gamma_config_flag == 1) {
		...
		for (index = 0; index < pinfo->gamma_lut_table_len / 2; index++) {
			...
			outp32(gamma_lut_base + (U_GAMA_R_COEF + index * 4),
				(local_gamma_lut_table_r[i] |
				(local_gamma_lut_table_r[i+1] << 16)));
			outp32(gamma_lut_base + (U_GAMA_G_COEF + index * 4),
				(local_gamma_lut_table_g[i] |
				(local_gamma_lut_table_g[i+1] << 16)));
			outp32(gamma_lut_base + (U_GAMA_B_COEF + index * 4),
				(local_gamma_lut_table_b[i] |
				(local_gamma_lut_table_b[i+1] << 16)));
			// GAMA  PRE LUT
		...
		outp32(gamma_lut_base + U_GAMA_R_LAST_COEF,
			local_gamma_lut_table_r[pinfo->gamma_lut_table_len - 1]);
		outp32(gamma_lut_base + U_GAMA_G_LAST_COEF,
			local_gamma_lut_table_g[pinfo->gamma_lut_table_len - 1]);
		outp32(gamma_lut_base + U_GAMA_B_LAST_COEF,
			local_gamma_lut_table_b[pinfo->gamma_lut_table_len - 1]);
		// GAMA PRE LUT
		outp32(gamma_lut_base + U_GAMA_PRE_R_LAST_COEF,
			local_gamma_lut_table_r[pinfo->gamma_lut_table_len - 1]);
		outp32(gamma_lut_base + U_GAMA_PRE_G_LAST_COEF,
			local_gamma_lut_table_g[pinfo->gamma_lut_table_len - 1]);
		outp32(gamma_lut_base + U_GAMA_PRE_B_LAST_COEF,
			local_gamma_lut_table_b[pinfo->gamma_lut_table_len - 1]);
	...
	const char __iomem *post_scf_base, dss_arsr1p_t *s_post_scf)
{
	...
	s_post_scf->dpp_img_size_bef_sr =
		(uint32_t)inp32(dss_base + DSS_DISP_CH0_OFFSET + IMG_SIZE_BEF_SR);
	s_post_scf->dpp_img_size_aft_sr =
		(uint32_t)inp32(dss_base + DSS_DISP_CH0_OFFSET + IMG_SIZE_AFT_SR);
	...
	dss_overlay_t *pov_req)
{
	...
	if ((ihinc > ARSR1P_INC_FACTOR) ||  (ivinc > ARSR1P_INC_FACTOR)) {
		...
		HISI_FB_ERR("scaling down is not supported by ARSR1P, ihinc = 0x%x, "
		"ivinc = 0x%x\n", ihinc, ivinc);
		// ARSR1P bypass
		post_scf->mode = 0x1;
		return -1;
...
int hisi_dss_mdc_module_default(struct hisi_fb_data_type *hisifd)
{
	...
	hisi_dss_aif_init(dss_mdc_module->aif_ch_base[DSS_RCHN_V0],
		&(dss_mdc_module->aif[DSS_RCHN_V0]));

	dss_mdc_module->mif_ch_base[DSS_RCHN_V0] = mdc_base + MIF_CH2_OFFSET;
	hisi_dss_mif_init(dss_mdc_module->mif_ch_base[DSS_RCHN_V0],
		&(dss_mdc_module->mif[DSS_RCHN_V0]), DSS_RCHN_V0);

	dss_mdc_module->mctl_ch_base[DSS_RCHN_V0].chn_mutex_base =
		mdc_base + MCTL_MUTEX_OFFSET + MCTL_CTL_MUTEX_RCH2;
	dss_mdc_module->mctl_ch_base[DSS_RCHN_V0].chn_ov_en_base =
		mdc_base + MCTL_SYS_OFFSET + MCTL_SYS_RCH_OEN;
	dss_mdc_module->mctl_ch_base[DSS_RCHN_V0].chn_flush_en_base =
		mdc_base + MCTL_SYS_OFFSET + MCTL_RCH2_FLUSH_EN;

	dss_mdc_module->mctl_ch_base[DSS_RCHN_V0].chn_mod_dbg_base =
		mdc_base + MCTL_SYS_OFFSET + MCTL_MOD2_DBG;
	hisi_dss_mctl_ch_mod_dbg_init(
		dss_mdc_module->mctl_ch_base[DSS_RCHN_V0].chn_mod_dbg_base,
		&(dss_mdc_module->mctl_ch[DSS_RCHN_V0]));

	dss_mdc_module->dma_base[DSS_RCHN_V0] = mdc_base + RCH_OFFSET;
	hisi_dss_rdma_init(dss_mdc_module->dma_base[DSS_RCHN_V0],
		&(dss_mdc_module->rdma[DSS_RCHN_V0]));
	hisi_dss_rdma_u_init(dss_mdc_module->dma_base[DSS_RCHN_V0],
		&(dss_mdc_module->rdma[DSS_RCHN_V0]));
	hisi_dss_rdma_v_init(dss_mdc_module->dma_base[DSS_RCHN_V0],
		&(dss_mdc_module->rdma[DSS_RCHN_V0]));

	dss_mdc_module->dfc_base[DSS_RCHN_V0] = mdc_base + DSS_RCH_VG0_DFC_OFFSET;
	hisi_dss_dfc_init(dss_mdc_module->dfc_base[DSS_RCHN_V0],
		&(dss_mdc_module->dfc[DSS_RCHN_V0]));

	// WCH
	dss_mdc_module->aif_ch_base[DSS_WCHN_W1] = mdc_base + AIF0_CH9_OFFSET;
	hisi_dss_aif_init(dss_mdc_module->aif_ch_base[DSS_WCHN_W1],
		&(dss_mdc_module->aif[DSS_WCHN_W1]));

	dss_mdc_module->mif_ch_base[DSS_WCHN_W1] = mdc_base + MIF_CH9_OFFSET;
	hisi_dss_mif_init(dss_mdc_module->mif_ch_base[DSS_WCHN_W1],
		&(dss_mdc_module->mif[DSS_WCHN_W1]), DSS_WCHN_W1);

	dss_mdc_module->mctl_ch_base[DSS_WCHN_W1].chn_mutex_base =
		mdc_base + MCTL_MUTEX_OFFSET + MCTL_CTL_MUTEX_WCH1;
	dss_mdc_module->mctl_ch_base[DSS_WCHN_W1].chn_ov_en_base =
		mdc_base + MCTL_SYS_OFFSET + MCTL_SYS_WCH_IEN;
	dss_mdc_module->mctl_ch_base[DSS_WCHN_W1].chn_flush_en_base =
		mdc_base + MCTL_SYS_OFFSET + MCTL_WCH1_FLUSH_EN;

	dss_mdc_module->mctl_ch_base[DSS_WCHN_W1].chn_mod_dbg_base =
		mdc_base + MCTL_SYS_OFFSET + MCTL_MOD9_DBG;
	hisi_dss_mctl_ch_mod_dbg_init(
		dss_mdc_module->mctl_ch_base[DSS_WCHN_W1].chn_mod_dbg_base,
		&(dss_mdc_module->mctl_ch[DSS_WCHN_W1]));

	dss_mdc_module->scl_base[DSS_WCHN_W1] = mdc_base + WCH_OFFSET + WCH_SCF_OFFSET;
	hisi_dss_scl_init(dss_mdc_module->scl_base[DSS_WCHN_W1],
		&(dss_mdc_module->scl[DSS_WCHN_W1]));

	dss_mdc_module->dma_base[DSS_WCHN_W1] = mdc_base + WCH_OFFSET;
	hisi_dss_wdma_init(dss_mdc_module->dma_base[DSS_WCHN_W1],
		&(dss_mdc_module->wdma[DSS_WCHN_W1]));

	dss_mdc_module->dfc_base[DSS_WCHN_W1] = mdc_base + WCH_OFFSET + WCH_DFC_OFFSET;
	hisi_dss_dfc_init(dss_mdc_module->dfc_base[DSS_WCHN_W1],
		&(dss_mdc_module->dfc[DSS_WCHN_W1]));

	dss_mdc_module->pcsc_base[DSS_WCHN_W1] = mdc_base + WCH_OFFSET + WCH_POST_CSC_OFFSET;
	hisi_dss_csc_init(dss_mdc_module->pcsc_base[DSS_WCHN_W1],
		&(dss_mdc_module->pcsc[DSS_WCHN_W1]));

	dss_mdc_module->post_clip_base[DSS_WCHN_W1] =
		mdc_base + WCH_OFFSET + WCH_POST_CLIP_OFFSET;
	hisi_dss_post_clip_init(dss_mdc_module->post_clip_base[DSS_WCHN_W1],
		&(dss_mdc_module->post_clip[DSS_WCHN_W1]));

	dss_mdc_module->csc_base[DSS_WCHN_W1] = mdc_base + WCH_OFFSET + WCH_CSC_OFFSET;
	hisi_dss_csc_init(dss_mdc_module->csc_base[DSS_WCHN_W1],
		&(dss_mdc_module->csc[DSS_WCHN_W1]));

	dss_mdc_module->mctl_sys_base = mdc_base + MCTL_SYS_OFFSET;
	hisi_dss_mctl_sys_init(dss_mdc_module->mctl_sys_base,
		&(dss_mdc_module->mctl_sys));
...
static void print_dsi_error_info(struct hisi_fb_data_type *hisifd,
	uint32_t *read_value)
{
	HISI_FB_ERR("fb%d, "
...
void hisi_dump_current_info(struct hisi_fb_data_type *hisifd)
{
	...
	for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
		...
		HISI_FB_INFO("chn%d DMA_BUF_DBG0=0x%x, DMA_BUF_DBG1=0x%x\n", i,
			inp32(hisifd->dss_base + g_dss_module_base[i][MODULE_DMA] +
				DMA_BUF_DBG0),
			inp32(hisifd->dss_base + g_dss_module_base[i][MODULE_DMA] +
				DMA_BUF_DBG1));
	}
	...
	HISI_FB_INFO("arsr_post: 0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x, "
		"0x%x,0x%x,0x%x, 0x%x\n",
	...
	HISI_FB_INFO("dpp: 0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x, "
	...
	if (hisifd->vactive0_start_flag == 0) {
		...
		while (1) {
			ret = wait_event_interruptible_timeout(hisifd->vactive0_start_wq,
				hisifd->vactive0_start_flag, msecs_to_jiffies(timeout_interval));
		...
		if (ret <= 0) {
			...
			ret1 = hisi_cmdlist_get_cmdlist_idxs(pov_req_prev,
				&cmdlist_idxs_prev, NULL);
			if (ret1 != 0)
				HISI_FB_INFO("fb%d, hisi_cmdlist_get_cmdlist_idxs pov_req_prev "
					"failed! ret = %d\n", hisifd->index, ret1);

			ret1 = hisi_cmdlist_get_cmdlist_idxs(pov_req_prev_prev,
			...
			if (g_debug_ldi_underflow || g_debug_ovl_online_composer_hold)
				hisi_dss_underflow_dump_cmdlist(hisifd, pov_req_prev,
					pov_req_prev_prev);
			...
			if (g_debug_ldi_underflow_clear && g_ldi_data_gate_en) {
				...
				hisifb_te_state_handle(hisifd, ldi_vstate, phy_status,
					panel_check, 1);
				return 0;
			}
			...
			while (1) {
				ret = wait_event_interruptible_timeout(
					hisifd->vactive0_start_wq, hisifd->vactive0_start_flag,
			...
			if (ret <= 0) {
				...
				ldi_vstate = inp32(mipi_dsi_base + MIPI_LDI_VSTATE);
				phy_status = inp32(mipi_dsi_base + MIPIDSI_PHY_STATUS_OFFSET);
				hisifb_te_state_handle(hisifd, ldi_vstate,
					phy_status, panel_check, 1);
			} else {
	...
	dss_overlay_t *pov_req)
{
	...
	if (is_mipi_cmd_panel(hisifd) && (hisifd->frame_update_flag == 0)) {
		...
		if (g_fastboot_enable_flag && g_enable_ovl_cmdlist_online)
			set_reg(hisifd->dss_base + DSS_MCTRL_CTL0_OFFSET +
				MCTL_CTL_TOP, 0x1, 32, 0);
