	...
	dss_wb_layer_t *layer, dss_rect_t aligned_rect, dss_rect_t *ov_block_rect, bool last_block)
{
	...
	#if defined(CONFIG_HISI_FB_V510) || defined(CONFIG_HISI_FB_V600)
		ret = hisi_dss_hebce_config(hisifd, pov_req, layer, aligned_rect, ov_block, last_block);
		if (ret != 0) {
...
int hisi_overlay_on(struct hisi_fb_data_type *hisifd, bool fastboot_enable)
{
	...
	if ((hisifd->index == PRIMARY_PANEL_IDX) ||
		...
		hisi_dss_mctl_on(hisifd, DPP_CMDLIST_MCTL, enable_cmdlist, fastboot_enable);
#endif
...
#ifdef DPP_CMDLIST_UTILS_SUPPORT
	hisi_dss_mctl_on(hisifd, DPP_CMDLIST_MCTL, enable_cmdlist, 0);
#endif
