...
void dump_dss_register_info(struct hisi_fb_data_type *hisifd,
	int ret, struct timeval tv0, struct timeval tv1)
{
	if (!hisifd) {
	...
	HISI_FB_ERR("fb%d, wait asyn vactive0 start flag timeout, ret=%d, "
		...
		hisifd->index, ret, hisifd->asynchronous_vactive0_start_flag,
		hisifd->vactive0_start_flag, hisifb_timestamp_diff(&tv0, &tv1),
		inp32(hisifd->mipi_dsi0_base + MIPI_LDI_CPU_ITF_INTS),
...
int hisi_get_release_and_retire_fence(struct hisi_fb_data_type *hisifd, void __user *argp)
{
	...
	if (is_mipi_cmd_panel(hisifd)) {
		...
		if (hisifd->asynchronous_vactive0_start_flag == 0) {
			...
			#if defined(CONFIG_HISI_FB_V510) || defined(CONFIG_HISI_FB_V350)
				dump_dss_register_info(hisifd, ret, tv0, tv1);
			#endif
				ret = -EFAULT;
				goto err_return;
			}
