...
// Internal API
//-------------------------------------------------------------------
static void dpp_dump_cmdlist_node_items(const cmd_item_t *item, uint32_t count)
{
	uint32_t index = 0;
	uint32_t addr = 0;
	...
	uint32_t item_num, uint8_t region, uint8_t buffer)
{
	...
	HISI_FB_INFO("\t qos  | flag | pending | tast_end | last  | event_list | list_addr  | next_list  | count | id | is_used | exec| intr\n");
	HISI_FB_INFO("\t------+---------+------------+------------+------------+------------\n");
	HISI_FB_INFO("\t 0x%2x | 0x%2x |0x%6x | 0x%6x | 0x%3x | 0x%8x | 0x%8x | 0x%8x | 0x%3x | 0x%1x| 0x%4x  | 0x%1x | 0x%1x\n",
...
static dss_cmdlist_node_t* dpp_cmdlist_node_alloc(struct hisi_fb_data_type *hisifd)
{
	...
	if (item_len < dpp_lut_size) {
		HISI_FB_ERR("cmdlist pool size:%d is not enough to store %d dpp lut %d\n",
...
static void dpp_cmdlist_reset_data(dpp_cmdlist_data_t *cmdlist_data)
{
	...
	if (node->list_header->total_items.bits.count > 0) {
		item_count = MIN(node->list_header->total_items.bits.count, node->item_len / sizeof(cmd_item_t));
		memset(node->list_item, 0, item_count * sizeof(cmd_item_t));
...
static void dpp_cmdlist_set_read_ptr(struct hisi_fb_data_type *hisifd, uint8_t region)
{
	...
	if (hisifd->dpp_cmdlist_data[region][buffer_rd]->buffer_status ==
		...
		HISI_FB_DEBUG("dpp_cmdlist_data set free: region=%d, buffer_rd=%d\n",
			region, buffer_rd);
...
static void dpp_cmdlist_set_reg(dss_cmdlist_node_t *node, uint32_t addr,
	uint32_t value, uint8_t bw, uint8_t bs)
{
	...
	if ((index + 1) >= (node->item_len / sizeof(cmd_item_t))) {
		HISI_FB_WARNING("index=%d is larger than %d!\n",
			(index + 1),
			(node->item_len / sizeof(cmd_item_t)));
		return;
	}
...
static uint32_t dpp_cmdlist_config_gmp(dss_cmdlist_node_t *node,
...
static uint32_t dpp_cmdlist_config_degamma(dss_cmdlist_node_t *node,
...
static uint32_t dpp_cmdlist_config_gamma(dss_cmdlist_node_t *node,
...
static void dpp_cmdlist_config_xcc(dss_cmdlist_node_t *node,
	uint32_t addr, uint32_t index,
		...
		if (hisifd_list[AUXILIARY_PANEL_IDX]->fb2_irq_force_on_flag == true) {
			if (hisifd_list[AUXILIARY_PANEL_IDX]->fb2_irq_on_flag == false) {
				...
				if (hisifd->panel_info.cascadeic_support) {
					...
					dpe_irq_disable(hisifd_list[AUXILIARY_PANEL_IDX]);
				}
			}
	...
	uint8_t config_region)
{
	...
	config_dpp = dpp_cmdlist_get_config_dpp(config_region);
	if (config_dpp >= DPP_CMDLIST_REGION_NUM) {
		HISI_FB_ERR("error config_dpp:%d", config_dpp);
		return;
	}
	...
	HISI_FB_DEBUG("config_dpp=%d, buffer_rd=%d, stutas=0x%x, config_region=%d",
		config_dpp, buffer_rd,
		hisifd->dpp_cmdlist_data[config_dpp][buffer_rd]->buffer_status,
		config_region);
	...
	if (hisifd->dpp_cmdlist_data[hisifd->dpp_cmdlist_region][last_buffer_rd]->buffer_status ==
		...
		reset = true;
		HISI_FB_INFO("reset last cmdlist to free: region=%d, buffer_rd=%d",
	...
	bool in_roi)
{
	...
	if (effect_info == NULL || buf_info == NULL) {
