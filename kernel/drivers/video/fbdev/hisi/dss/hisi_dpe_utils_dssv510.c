...
static int dss_inner_clk_refcount;
...
#endif
extern int effect_dpp_ch_config_roi(struct hisi_fb_data_type *hisifd,
...
static int set_mdc_core_clk(struct hisi_fb_data_type *hisifd,
	dss_vote_cmd_t vote_cmd)
{
	...
	HISI_FB_INFO_INTERIM("Trying to set dss_mmbuf_clk from %llu to %llu\n",
...
static int dss_core_clk_enable(struct hisi_fb_data_type *hisifd)
{
	...
	if (hisifd->dss_axi_clk != NULL) {
		...
		if (ret) {
			HISI_FB_ERR("fb%d dss_axi_clk enable failed, error=%d!\n",
				hisifd->index, ret);
			return -EINVAL;
		}
	}
	...
	if (hisifd->dss_mmbuf_clk != NULL) {
		...
		if (ret) {
			HISI_FB_ERR("fb%d dss_mmbuf_clk enable failed, error=%d!\n",
				hisifd->index, ret);
			return -EINVAL;
		}
	}
...
static int hisifb_set_edc_mmbuf_clk(struct hisi_fb_data_type *hisifd,
	dss_vote_cmd_t target_vote)
{
	int ret = 0;
	...
	HISI_FB_INFO_INTERIM("Trying to set dss_pri_clk from %llu to %llu\n",
		clk_get_rate(hisifd->dss_pri_clk), target_vote.dss_pri_clk_rate);
	...
	dss_vote_cmd_t vote_cmd)
{
	...
	if ((hisifd->index != AUXILIARY_PANEL_IDX) &&
		...
		if (!hisifd->panel_info.cascadeic_support) {
			...
			if (vote_cmd.dss_mmbuf_rate == hisifd->dss_vote_cmd.dss_mmbuf_rate)
				return ret;
		}
	}
	...
	uint64_t pixel_clock)
{
	...
	if (hisifd->index == EXTERNAL_PANEL_IDX) {
		...
		if ((voltage_value > PERI_VOLTAGE_LEVEL0_060V) &&
			(pixel_clock > DEFAULT_DSS_CORE_CLK_RATE_L1)) {
...
int dpe_set_clk_rate(struct platform_device *pdev)
{
	...
	tmp.dss_pri_clk_rate =
		tmp.dss_pri_clk_rate > other_fb_clk_rate.dss_pri_clk_rate ?
		tmp.dss_pri_clk_rate : other_fb_clk_rate.dss_pri_clk_rate;
	...
	if ((hisifd->index == PRIMARY_PANEL_IDX) ||
		...
		HISI_FB_INFO("fb%d, dss_pri_clk set %llu get %llu\n",
			hisifd->index, tmp.dss_pri_clk_rate,
			(uint64_t)clk_get_rate(hisifd->dss_pri_clk));
...
static void lp_second_level_clk_gate_ctrl(struct hisi_fb_data_type *hisifd)
{
	...
	inheritor_dss_wr_reg(dss_base + DSS_DISP_CH0_OFFSET +
		DISP_CH_CLK_EN, 0xFFFF7FFF);
	...
	inheritor_dss_wr_reg(dss_base + DSS_RCH_VG0_DMA_OFFSET +
		CH_CLK_EN, 0xFFFF7FFF);
	outp32(dss_base + DSS_RCH_VG0_DMA_OFFSET + FBCD_CREG_FBCD_CTRL_GATE,
		0x0000000C);
	outp32(dss_base + DSS_RCH_VG1_DMA_OFFSET + CH_CLK_SEL, 0x00000000);
	outp32(dss_base + DSS_RCH_VG1_DMA_OFFSET + FBCD_CREG_FBCD_CTRL_GATE,
		0x0000000C);
	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_RCH_VG2_DMA_OFFSET,
		CH_CLK_SEL, 0x00000000);
	outp32(dss_base + DSS_RCH_G0_DMA_OFFSET + CH_CLK_SEL, 0x00000000);
	outp32(dss_base + DSS_RCH_G0_DMA_OFFSET + FBCD_CREG_FBCD_CTRL_GATE,
		0x0000000C);
	outp32(dss_base + DSS_RCH_G1_DMA_OFFSET + CH_CLK_SEL, 0x00000000);
	outp32(dss_base + DSS_RCH_G1_DMA_OFFSET + FBCD_CREG_FBCD_CTRL_GATE,
		0x0000000C);
	...
	outp32(dss_base + DSS_RCH_D0_DMA_OFFSET + FBCD_CREG_FBCD_CTRL_GATE,
		0x0000000C);
	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_RCH_D1_DMA_OFFSET,
		CH_CLK_SEL, 0x00000000);
	...
	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_OVL3_OFFSET,
		OV2_CLK_SEL, 0x00000000);
...
static void no_memory_lp_ctrl(struct hisi_fb_data_type *hisifd)
{
	...
	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_RCH_VG1_SCL_OFFSET,
		SCF_COEF_MEM_CTRL, 0x00000088);

	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_RCH_VG1_SCL_OFFSET,
		SCF_LB_MEM_CTRL, 0x00000008);
	...
	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_RCH_G1_SCL_OFFSET,
		SCF_COEF_MEM_CTRL, 0x00000088);
	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_RCH_G1_SCL_OFFSET,
		SCF_LB_MEM_CTRL, 0x0000008);
	...
	// dssv350 not support this module
	dss_set_reg_inheritor(dss_base, DSS_HDR_OFFSET,
		HDR_MEM_CTRL, 0x08888888);
...
static void init_dsc(struct hisi_fb_data_type *hisifd)
{
	...
	rbs_min = dsc->rc_model_size - dsc->initial_offset +
	...
	num_extra_mux_bits = 3 * (dsc->mux_word_size +
		(4 * dsc->bits_per_component + 4) - 2);
	...
	final_offset =	dsc->rc_model_size -
		...
		num_extra_mux_bits; // 4336(0x10f0)
	final_scale = 8 * dsc->rc_model_size / (dsc->rc_model_size - final_offset);
	nfl_bpg_offset =
		round1(dsc->first_line_bpg_offset << OFFSET_FRACTIONAL_BITS,
		dsc->slice_height); // 793(0x319)
	...
	slices_per_line =
		((pic_width + 1) / (dual_dsc_en + 1)) / (dsc->slice_width + 1) - 1;
	...
	set_reg(dsc_base + DSC_RC_PARAM1,
		(dsc->first_line_bpg_offset << 16) | scale_decrement_interval, 21, 0);
...
void init_dbuf(struct hisi_fb_data_type *hisifd)
{
	...
	if (dbuf_burst) {
		...
		thd_cg_in = depth - 1;
		htotal = pinfo->xres + (pinfo->ldi.h_pulse_width +
			pinfo->ldi.h_back_porch + pinfo->ldi.h_front_porch) *
			pinfo->pxl_clk_rate_div;
		vtotal = pinfo->yres + pinfo->ldi.v_pulse_width +
...
void ldi_frame_update(struct hisi_fb_data_type *hisifd, bool update)
{
	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		if (is_mipi_cmd_panel(hisifd)) {
			set_reg(hisifd->mipi_dsi0_base + MIPI_LDI_FRM_MSK,
				(update ? 0x0 : 0x1), 1, 0);
			if (is_dual_mipi_panel(hisifd))
	...
	if (param_choice == 1) {
		...
		if (gama_enable) {
			for (i = 0; i < GAMA_LUT_LENGTH; i = i + 2) {
				set_reg(gamma_lut_base + (U_GAMA_R_COEF + i * 2),
					gama->gama_r_lut[i], 12, 0);
				if (i != GAMA_LUT_LENGTH - 1)
					set_reg(gamma_lut_base + (U_GAMA_R_COEF + i * 2),
						gama->gama_r_lut[i + 1], 12, 16);

				set_reg(gamma_lut_base + (U_GAMA_G_COEF + i * 2),
					gama->gama_g_lut[i], 12, 0);
				if (i != GAMA_LUT_LENGTH - 1)
					set_reg(gamma_lut_base + (U_GAMA_G_COEF + i * 2),
						gama->gama_g_lut[i + 1], 12, 16);

				set_reg(gamma_lut_base + (U_GAMA_B_COEF + i * 2),
					gama->gama_b_lut[i], 12, 0);
				if (i != GAMA_LUT_LENGTH - 1)
					set_reg(gamma_lut_base + (U_GAMA_B_COEF + i * 2),
						gama->gama_b_lut[i + 1], 12, 16);
			}
...
void init_igm_gmp_xcc_gm(struct hisi_fb_data_type *hisifd)
{
	...
	work_channel = (uint32_t)inp32(hisifd->dss_base +
		DSS_DISP_GLB_OFFSET + DYN_SW_DEFAULT) & 0x1;
	HISI_FB_INFO("[effect] inited=%d buf_sel=%d, buf_status:gmp=%d,gama=%d,"
		"degama=%d,xcc=%d, work_ch=%d, ch_status=%d,%d, buff_sel_roi=%d\n",
		is_init,
		buff_sel,
		...
		work_channel,
		*(hisifd->effect_info.dpp_chn_status + work_channel),
		*(hisifd->effect_info.dpp_chn_status + ((~work_channel) & 0x1)),
		buff_sel_roi);

	if (buff_sel_roi < DPP_BUF_MAX_COUNT)
		HISI_FB_INFO("buf_status_roi:gmp=%d,gama=%d,degama=%d,xcc=%d\n",
		dpp_buff_mngr->buf_info_list[buff_sel_roi].gmp_buf_cfg_status,
		dpp_buff_mngr->buf_info_list[buff_sel_roi].gama_buf_cfg_status,
		dpp_buff_mngr->buf_info_list[buff_sel_roi].degama_buf_cfg_status,
		dpp_buff_mngr->buf_info_list[buff_sel_roi].xcc_buf_cfg_status);
	...
	if (is_init) {
		...
		if (dpp_buff_mngr->buf_work_mode == DPP_BUF_WORK_MODE_BOTH_EFFECT) {
			...
			hisifd->effect_info.dpp_buf.roi_new_updated = true;
	...
	//if dpp0&dpp1 gmp inited successfully, gmp_call_wq_scene should be 0.
...
//lint +e838 +e550 +e438 +e737
/*lint -e838*/
void dpe_store_ct_csc_value(struct hisi_fb_data_type *hisifd,
	unsigned int csc_value[])
{
	struct hisi_panel_info *pinfo = NULL;
...
int dpe_set_ct_csc_value(struct hisi_fb_data_type *hisifd)
{
	...
	if (pinfo->color_temp_rectify_support &&
