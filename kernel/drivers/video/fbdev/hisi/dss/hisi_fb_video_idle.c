	...
	dss_layer_t *layer)
{
	...
	if ((LB_AVAILABLE_SIZE > 0) &&
		(layer->img.buf_size > video_idle_ctrl->l3cache_size))
		video_idle_ctrl->lb_size =
		layer->img.buf_size - video_idle_ctrl->l3cache_size;
}
...
static int hisifb_video_idle_alloc_cache(struct hisi_fb_data_type *hisifd)
{
	...
	ret = dma_buf_begin_cpu_access(attach->dmabuf, DMA_FROM_DEVICE);
	if (ret) {
