	...
	struct sock *sk, u8 protocol)
{
	...
	if (time_before(sk->sk_born_stamp, rule->stamp)) {
		hwlog_info("%s: sk->sk_born_stamp = %d, rule->stamp= %d\n",
...
static bool is_rule_in_list(u32 net_id)
{
...
int slice_rules_lookup(struct sock *sk, struct sockaddr *uaddr, u8 protocol)
{
	...
	if (sk->sk_slice_binded) {
		hwlog_info("%s: sk_slice_binded, sk_mark = %d\n",
			__func__, sk->sk_mark);
		if (is_rule_in_list(sk->sk_mark & 0xFFFF)) {
			return 0;
		}
		else {
...
static int add_rule_to_list(struct slice_route_rule *new_rule)
{
	...
	if (is_rule_in_list(new_rule->net_id))
		return -EINVAL;
...
int add_uid_netid_to_rule_list(u32 uid, u32 net_id)
{
	struct slice_route_rule *rule = NULL;
...
static int add_addr_netid_to_rule_list(struct remote_address *addr, u32 net_id)
{
	struct slice_route_rule *rule = NULL;
	struct slice_route_rule *temp = NULL;
	int err = -EINVAL;
...
static int del_uid_netid_from_rule_list(u32 uid, u32 net_id)
{
	struct slice_route_rule *rule = NULL;
	struct slice_route_rule *temp = NULL;
	int err = -EINVAL;
	...
	struct slice_route_rule *route_rule, char **pos, int len)
{
	...
	if (msg_len < 1 + 8 * ipv4_num)
		return -EINVAL;
	msg_len = msg_len - (1 + 8 * ipv4_num);
	skip_byte(position, 1);
	for (i = 0; i < ipv4_num; i++) {
		...
		memcpy(&ipv4_addr->addr_u.ip4_addr.addr, position, 4);
		skip_byte(position, 4);
		memcpy(&ipv4_addr->addr_u.ip4_addr.mask, position, 4);
		skip_byte(position, 4);
	...
	struct slice_route_rule *route_rule, char **pos, int len)
{
	...
	if (msg_len < 1 + 17 * ipv6_num)
		return -EINVAL;
	msg_len = msg_len - (1 + 17 * ipv6_num);
	skip_byte(position, 1);
	for (i = 0; i < ipv6_num; i++) {
		...
		memcpy(&ipv6_addr->addr_u.ip6_addr.addr, position, 16);
		skip_byte(position, 16);
	...
	struct slice_route_rule *route_rule, char **pos, int len)
{
	...
	if (msg_len < 1 + 4 * port_range_num)
		return -EINVAL;
	msg_len = msg_len - (1 + 4 * port_range_num);
...
static int add_uid_to_exist_rule_list(char **pos, u32 net_id, int len)
{
	u32 uid_num;
	u32 uid;
	u32 i;
	...
	skip_byte(position, 1);
	for (i = 0; i < uid_num; i++) {
		memcpy(&uid, position, 4);
		if (add_uid_netid_to_rule_list(uid, net_id) == -EINVAL)
			return -EINVAL;
		skip_byte(position, 4);
	}
	*pos = position;
...
static int add_ipv4_to_exist_rule_list(char **pos, u32 net_id, int len)
{
	u32 i;
	u8 ipv4_num;
	char *position = *pos;
	...
	for (i = 0; i < ipv4_num; i++) {
		...
		memcpy(&ipv4_addr->addr_u.ip4_addr.mask, position, 4);
		skip_byte(position, 4);
		if (add_addr_netid_to_rule_list(ipv4_addr, net_id) != 0) {
			kfree(ipv4_addr);
			return -EINVAL;
		}
	}
...
static int add_ipv6_to_exist_rule_list(char **pos, u32 net_id, int len)
{
	u32 i;
	u8 ipv6_num;
	char *position = *pos;
	...
	for (i = 0; i < ipv6_num; i++) {
		...
		ipv6_addr->addr_u.ip6_addr.prefix_len = *position;
		skip_byte(position, 1);
		if (add_addr_netid_to_rule_list(ipv6_addr, net_id) != 0) {
			kfree(ipv6_addr);
			return -EINVAL;
		}
	}
...
void bind_process_to_slice(struct req_msg_head *msg)
{
	...
	if (is_rule_in_list(net_id)) {
		hwlog_info("%s: net id %d is in list\n", __func__, net_id);
		len = add_uid_to_exist_rule_list(&position, net_id, len);
		if (len < 0)
			return;
		len = add_ipv4_to_exist_rule_list(&position, net_id, len);
		if (len < 0)
			return;
		len = add_ipv6_to_exist_rule_list(&position, net_id, len);
		return;
	}
...
void unbind_process_to_slice(struct req_msg_head *msg)
{
	...
	// msg len < 2B(type) + 2B(len) + 4B(net id)
	if (len < 8)
		return;
	len -= 8;
	skip_byte(position, 4);
	...
	// delete uid
	hwlog_info("%s: uid_num = %d\n", __func__, uid_num);
	for (i = 0; i < uid_num; i++) {
		memcpy(&uid, position, 4);
		skip_byte(position, 4);
		del_uid_netid_from_rule_list(uid, net_id);
	}
}
