...
oal_int32 pcie_auto_disable_aspm = 0;
oal_debug_module_param(pcie_auto_disable_aspm, int, S_IRUGO | S_IWUSR);
...
#ifdef CONFIG_ARCH_KIRIN_PCIE
OAL_STATIC oal_void oal_kirin_pcie_deregister_event(oal_pcie_linux_res *pst_pci_lres)
{
#ifndef _PRE_HI375X_PCIE
...
OAL_STATIC oal_void oal_kirin_pcie_register_event(oal_pci_dev_stru *pst_pci_dev, oal_pcie_linux_res *pst_pci_lres)
{
#ifndef _PRE_HI375X_PCIE
...
OAL_STATIC oal_int32 oal_pcie_bindcpu(hcc_bus *pst_bus, oal_uint32 chan, oal_int32 is_bind)
{
    ...
    if (is_bind) {
        ...
        oal_print_hi11xx_log(HI11XX_LOG_DBG, "pcie bindcpu irq =%u, bind to cpu %d", pst_pci_dev->irq, OAL_BUS_HPCPU_NUM);
        irq_set_affinity_hint(pst_pci_dev->irq, cpumask_of(OAL_BUS_HPCPU_NUM));
#else
        oal_print_hi11xx_log(HI11XX_LOG_DBG, "pcie bindcpu fail, cpu:%d irq:%u", CONFIG_NR_CPUS, pst_pci_dev->irq);
#endif
#endif
...
OAL_STATIC oal_int32 oal_pcie_pm_control(struct pci_dev *pdev, u32 rc_idx, int power_option)
{
...
OAL_STATIC oal_int32 oal_pcie_power_notifiy_register(struct pci_dev *pdev, u32 rc_idx, int (*poweron)(void *data),
                                         ...
                                         int (*func)(void *data), oal_void *data)
{
    ...
    if (ctrl == HCC_BUS_CTRL_POWER_UP) {
        ret = oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, func, NULL, data);
    }

    if (ctrl == HCC_BUS_CTRL_POWER_DOWN) {
        ret = oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, NULL, func, data);
    }
...
OAL_STATIC oal_int32 oal_enable_pcie_irq_with_request(oal_pcie_linux_res *pst_pci_lres)
{
    oal_int32 ret = OAL_SUCC;
    if (OAL_WARN_ON(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    ...
    if (pst_pci_lres->irq_stat == 1) {
        if (!pst_pci_lres->st_msi.is_msi_support) {
            ...
            ret = request_irq(pst_pci_lres->pst_pcie_dev->irq, oal_pcie_intx_isr, IRQF_SHARED,
                              "hisi_pci_intx", (oal_void *)pst_pci_lres);
            if (ret < 0) {
...
oal_int32 oal_disable_pcie_irq_with_free(oal_pcie_linux_res *pst_pci_lres)
{
    if (OAL_WARN_ON(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }
...
oal_void oal_pcie_power_down(hcc_bus *pst_bus)
{
    ...
    /* 下电之前关闭 PCIE HOST 控制器 */
    {
        oal_int32 ret;
#ifdef _PRE_PRODUCT_HI1620S_KUNPENG
        ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, 0); /* power off with L3 message */
#else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0))
        ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, 2); /* power off without L3 message */
#else
        ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, 0); /* power off with L3 message */
#endif
#endif
        if (old_power_status != PCIE_EP_IP_POWER_DOWN) {
            if (ret) {
                PCI_PRINT_LOG(PCI_LOG_INFO, "pcie_pm_control failed=%d\n", ret);
                DECLARE_DFT_TRACE_KEY_INFO("pcie_power_down_fail", OAL_DFT_TRACE_EXCEP);
            }
        }
    }

    oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, NULL, NULL, NULL);
#endif
}
...
OAL_STATIC void oal_pcie_enable_irq_for_ppatch_launch(oal_pcie_linux_res *pst_pci_lres)
{
    if (OAL_WARN_ON(pst_pci_lres == NULL)) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "pst_pci_lres is null.\n");
        return;
    }
...
oal_int32 oal_free_pcie_irq_for_s4(oal_pcie_linux_res *pst_pci_lres)
{
    if (OAL_WARN_ON(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    PCI_PRINT_LOG(PCI_LOG_INFO, "diable_and_then_free_pcie_irq_for_s4");
...
void oal_free_irq_in_s4(void)
{
    ...
    uint32 bfg_irq;
    BOARD_INFO *board = NULL;

    PCI_PRINT_LOG(PCI_LOG_INFO, "free_irq_in_s4..\n");

    if (pcie_linux_res == NULL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "pcie_linux_res is null.\n");
        return;
    }

    pst_bus = pcie_linux_res->pst_bus;
    if (pst_bus == NULL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "pst_bus is null.\n");
        return;
    }
    ...
    //free pcie intx
    oal_disable_pcie_irq(pst_pci_lres);
    oal_free_pcie_irq_for_s4(pst_pci_lres);

    // free wlan irq
    pst_bus_dev = HBUS_TO_DEV(pst_bus);
    free_irq(pst_bus_dev->ul_wlan_irq, pst_bus_dev);
    //free bfgx irq
    board = get_hi110x_board_info_etc();
    if (board == NULL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "board is null.\n");
        return;
    }

    bfg_irq = board->bfgx_irq;
    PCI_PRINT_LOG(PCI_LOG_INFO, "bfg_irq is %d.\n", bfg_irq);
    free_irq(board->bfgx_irq, NULL);
    return;
}
oal_int32 oal_request_pcie_irq_for_s4(oal_pcie_linux_res *pst_pci_lres)
{
    oal_int32 ret;
    if (OAL_WARN_ON(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    ...
    /* intx */
    PCI_PRINT_LOG(PCI_LOG_INFO, "request_pcie_irq_for_s4");
    ret = request_irq(pst_pci_lres->pst_pcie_dev->irq, oal_pcie_intx_isr, IRQF_SHARED,
        "hisi_pci_intx", (oal_void *)pst_pci_lres);
    if (ret < 0) {
        OAL_IO_PRINT("enable_pcie_irq with request_irq error\n");
...
void request_wlan_gpio_irq_in_s4(hcc_bus_dev *pst_bus_dev)
{
    ...
    if (pst_bus_dev == NULL) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "pst_bus_dev is null.\n");
        return;
    }
    ...
    if (ret < 0) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "wlan_gpio_irq_request_for_s4 request_irq fail.\n");
        return;
    }
    disable_irq_nosync(pst_bus_dev->ul_wlan_irq);
    pst_bus_dev->ul_irq_stat = 1;/* irq disabled default. */
    return;
}
...
void request_bfgx_gpio_irq_in_s4(void)
{
    ...
    bfg_irq = board->bfgx_irq;
    PCI_PRINT_LOG(PCI_LOG_INFO, "request bfgx irq %d in s4\n", bfg_irq);
    ret = request_irq(bfg_irq, bfg_wake_host_isr_etc, IRQF_DISABLED | IRQF_TRIGGER_RISING | IRQF_NO_SUSPEND,
        "bfgx_wake_host", NULL);
    if (ret < 0) {
        PCI_PRINT_LOG(PCI_LOG_ERR, "bfgx_power_on_request_irq_s4 fail.\n");
        return;
    }
...
void oal_request_irq_in_s4(void)
{
    ...
    PCI_PRINT_LOG(PCI_LOG_INFO, "request_irq_in_s4..\n");
    ...
    //request pcie intx irq
    oal_request_pcie_irq_for_s4(pst_pci_lres);
    oal_disable_pcie_irq(pst_pci_lres);
...
OAL_STATIC oal_int32 oal_pcie_power_action(hcc_bus *pst_bus, HCC_BUS_POWER_ACTION_TYPE action)
{
    ...
    if (action == HCC_BUS_POWER_UP || action == HCC_BUS_SW_POWER_UP) {
        ...
        ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, 1);
        if (pcie_print_once) {
            pcie_print_once = 0;
            ssi_dump_device_regs(SSI_MODULE_MASK_COMM | SSI_MODULE_MASK_PCIE_CFG |
                 SSI_MODULE_MASK_PCIE_DBI | SSI_MODULE_MASK_WCTRL | SSI_MODULE_MASK_BCTRL);
        }
        board_host_wakeup_dev_set(0);
        oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, NULL, NULL, NULL);
        if (ret) {
    ...
    if (action == HCC_BUS_POWER_PATCH_LAUCH) {
        ...
        oal_pcie_enable_irq_for_ppatch_launch(pst_pci_lres);
...
#ifdef CONFIG_KIRIN_PCIE_L1SS_IDLE_SLEEP
OAL_STATIC oal_void oal_pcie_wlan_pm_vote(hcc_bus *hi_bus, oal_uint8 uc_allow)
{
    /* PCIe 32K feature, control rc fnpll clock */
    /* 0 means allow to fnpll gating */
    /* 1 means disallow to fnpll gating */
    kirin_pcie_refclk_device_vote(PCIE_DEVICE_WLAN, kirin_rc_idx,
                                   (uc_allow == ALLOW_IDLESLEEP) ? 0 : 1);
}
#endif
...
OAL_STATIC oal_int32 oal_pci_resource_init(oal_void)
{
    oal_int32 ret;
    oal_pcie_linux_res *pst_pci_lres = NULL;
...
#if defined(CONFIG_ARCH_KIRIN_PCIE) || defined(_PRE_CONFIG_ARCH_HI1620S_KUNPENG_PCIE)
        oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, kirin_rc_idx, NULL,
...
OAL_STATIC oal_void oal_pcie_110x_enum_fail_proc(oal_void)
{
...
OAL_STATIC oal_int32 oal_wait_pcie_probe(oal_void)
{
    oal_int32 ret = OAL_SUCC;
