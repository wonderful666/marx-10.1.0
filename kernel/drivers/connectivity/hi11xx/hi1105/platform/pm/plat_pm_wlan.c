...
#else
void wlan_pm_wdg_timeout_etc(struct timer_list *t);
#endif
...
static int wlan_poweroff_notify_sys(struct notifier_block *this, unsigned long code, void *unused)
{
    ...
    if (board_get_wlan_enable_gpio_val_etc()) {
        ...
        err = wlan_pm_close_etc();

        if (err != OAL_SUCC) {
            PS_PRINT_WARNING("wlan_poweroff_notify_sys call wlan_pm_close_etc failed (err=%d)\n", err);
        }
    } else {
        PS_PRINT_INFO("wlan already poweroff no call wlan_pm_close_etc\n");
    }

    return NOTIFY_OK;
}

static struct notifier_block wlan_poweroff_notifier = {
    .notifier_call = wlan_poweroff_notify_sys,
};
#endif
OAL_STATIC void wlan_pm_init_ext2_etc(struct wlan_pm_s *pst_wlan_pm)
{
#ifdef _PRE_CONFIG_HISIFASTPOWER_DOWN_STATE
    int32 err;
#endif
    ...
    wlan_pm_dts_init();
#ifdef _PRE_CONFIG_HISIFASTPOWER_DOWN_STATE
    err = register_reboot_notifier(&wlan_poweroff_notifier);
    if (err) {
...
OAL_STATIC int32 wlan_pm_init_ext1_etc(struct wlan_pm_s *pst_wlan_pm)
{
...
#else
    timer_setup(&pst_wlan_pm->st_watchdog_timer, wlan_pm_wdg_timeout_etc, 0);
#endif
...
struct wlan_pm_s *wlan_pm_init_etc(oal_void)
{
    ...
    /* 圈复杂度优化 */
    l_ret = wlan_pm_init_ext1_etc(pst_wlan_pm);
    if (l_ret == OAL_PTR_NULL) {
        return OAL_PTR_NULL;
    }

    wlan_pm_init_ext2_etc(pst_wlan_pm);
...
#ifdef _PRE_CONFIG_HISIFASTPOWER_DOWN_STATE
    unregister_reboot_notifier(&wlan_poweroff_notifier);
#endif
...
oal_void wlan_pm_idle_sleep_vote(oal_uint8 uc_allow)
{
    ...
    hcc_bus_wlan_pm_vote(hcc_get_current_110x_bus(), uc_allow);
#endif
#endif
}
