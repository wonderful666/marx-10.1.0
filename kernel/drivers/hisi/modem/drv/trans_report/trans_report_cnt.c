...
#define WAN_TRANS_TIMEOUT (1000) /* ms unit */
...
#ifdef CONFIG_MAA_BALONG
struct maa_debug_info maa_dbg_info;
#endif

#ifdef CONFIG_BALONG_ESPE
struct espe_om_info epse_dbg_info;
#endif

#ifdef CONFIG_WAN
struct wan_debug_info_s wan_dbg_info;
#endif
...
void bsp_wan_transreport(unsigned long data)
{
    ...
    size = bsp_maa_hds_transreport(&maa_dbg_info);
    if (size >= 0) {
        sTransInd.ulLength = sizeof(struct maa_debug_info);
        sTransInd.pData = (void *)(&maa_dbg_info);
    ...
    size = bsp_espe_get_om_info(&epse_dbg_info);
    if (size >= 0) {
        sTransInd.ulLength = size;
        sTransInd.pData = (void *)(&epse_dbg_info);
        ret = bsp_transreport(&sTransInd);
        if (ret != HDS_TRANS_RE_SUCC) {
            wan_trans->transreport_error++;
        }
    ...
    size = wan_transreport(&wan_dbg_info);
    if (size >= 0) {
        sTransInd.ulLength = sizeof(struct wan_debug_info_s);
        sTransInd.pData = (void *)(&wan_dbg_info);
        ret = bsp_transreport(&sTransInd);
        if (ret != HDS_TRANS_RE_SUCC) {
            wan_trans->transreport_error++;
        }
