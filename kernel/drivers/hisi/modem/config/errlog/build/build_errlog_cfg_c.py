...
subprocess.call("title errlog xml decode~", shell=True)
...
def decode_xml_by_faultId(dict):
    ...
    for fault_id_name in dd:
        ...
        if ('' != str_cdma):
                str_zhushi_s = '%s%s' %(' /*Item:%d' %arrycount[0], ' */')
                str_cdma_all = '%s%s%s%s%s' %(str_cdma_all, '\r\n    ', fault_id_name[1], str_zhushi_s, '\r\n', str_cdma)
        if ('' != str1_cdma):
                str_zhushi_s = '%s%s' %(' /*Item:%d'%arrycount[1], ' */')
                str1_cdma_all = '%s%s%s%s%s%s' %(str1_cdma_all, '\r\n    ', fault_id_name[1], str_zhushi_s, '\r\n', str1_cdma)
        if ('' != str2_cdma):
                str_zhushi_s = '%s%s' %(' /*Item:%d'%arrycount[2], ' */')
                str2_cdma_all = '%s%s%s%s%s%s' %(str2_cdma_all, '\r\n    ', fault_id_name[1], str_zhushi_s, '\r\n', str2_cdma)

        if ('' != str_gu):
                str_zhushi_s = '%s%s' %(' /*Item:%d'%arrycount[3], ' */')
                str_gu_all = '%s%s%s%s%s%s' %(str_gu_all, '\r\n    ', fault_id_name[1], str_zhushi_s, '\r\n', str_gu)
        if ('' != str1_gu):
                str_zhushi_s = '%s%s' %(' /*Item:%d'%arrycount[4], ' */')
                str1_gu_all = '%s%s%s%s%s%s' %(str1_gu_all, '\r\n    ', fault_id_name[1], str_zhushi_s, '\r\n', str1_gu)
        if ('' != str2_gu):
                str_zhushi_s = '%s%s' %(' /*Item:%d'%arrycount[5], ' */')
                str2_gu_all = '%s%s%s%s%s%s' %(str2_gu_all, '\r\n    ', fault_id_name[1], str_zhushi_s, '\r\n', str2_gu)
    ...
    '''
     ...
     修改历史      :
      1.日    期   : 2016年2月18日
        ...
        修改内容   : ERR LOG FAULT ID关联项目新增
    '''
    with open(PATH_ERR_LOG_CFG_H, 'rb') as includefp:
    ...
    '''
     ...
     修改历史      :
      1.日    期   : 2016年2月18日
        ...
        修改内容   : ERR LOG FAULT ID关联项目新增
    '''
    with open(PATH_ERR_LOG_CFG_C, 'rb') as includefp:
        str_hash_valude=''
         faultidArry = includefp.read()
    ...
    '''
     ...
     修改历史      :
      1.日    期   : 2016年2月18日
        ...
        修改内容   : ERR LOG FAULT ID关联项目新增
    '''
    with open(PATH_ERR_LOG_CFG_C, 'rb') as includefp_rb:
        str_hash_valude=''
        faultidArry=''
        faultidArry = includefp_rb.read()
        faultidArry = faultidArry.replace(str_old, str_new, 1);

    with open(PATH_ERR_LOG_CFG_C, 'wb') as includefp_wb:
        includefp_wb.write(faultidArry)
...
def write_file_cfg(write_count):
    ...
    #重写.c
    with open(PATH_ERR_LOG_CFG_C, 'wb') as includefp:
...
def calc_xml_file_hash():
    ...
    str_xml_value=''
    for path_xml in  PATH_ERR_LOG_XML_ARRY:
        with open(path_xml, 'rb') as xml_fp:
