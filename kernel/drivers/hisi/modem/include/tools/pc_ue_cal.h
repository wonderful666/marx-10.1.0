...
typedef UINT16 CBT_BUSINESS_TYPE_ENUM_UINT16;
...
typedef UINT16 CBT_NosigTypeUint16;

typedef struct {
    UINT8 ucModem : 3;  // modem0: 0  modem1: 1
    UINT8 ucResv : 1;
    UINT8 ucSsid : 4;  // ACPU: 0x1, CCPU:0x2,LTE DSP:0x3, GU DSP:0x5, TDS DSP:0x7, XDSP:0xe
} MODEM_SSID_STRU;

typedef struct {
    UINT8 ucMsgType : 2;        // 消息类型，REQ: 1   CNF: 2   IND:3
    UINT8 ucFragmentIndex : 4;  // 消息分段的片段索引,0...15
    UINT8 ucEof : 1;            // 分段结束标识，0：分段未结束，1：分段结束
    UINT8 ucFragmentFlag : 1;   // 是否分段标识，0不分段，1分段
} MSG_FRAGMENT_INFO_STRU;
...
typedef struct _OM_Msg_Socp_Head_Stru {
    UINT8                  ucSid;  // 固定为7
    MODEM_SSID_STRU        stModemSsid;
    UINT8                  ucSessionID;  // 固定为1
    MSG_FRAGMENT_INFO_STRU stMsgSegment;
    UINT32                 ulTransId;
    TIME_STAMP_STRU        stTimeStamp;
...
typedef struct _Component_Id {
    UINT8 ucRsv;
    UINT8 ucComponentType : 4;   /* 0x0: LTE, 0x1: TDS, 0x2: GSM, 0x3: UMTS, 0x4: CDMA, 0x5: NR, 0x7: LteV, 0xF: 与模无关 */
    UINT8 ucComponentID   : 4;   /* OM/MSP:0x1, nosig:0x2, PHY:0x3 */
} COMPONENT_ID_STRU;
...
typedef struct {
    ...
    UINT16                        usRfChanType : 4; /* 0:业务       1:侦听  只在GUC BT下有效 */
    UINT16                        usModeType : 4;   /* 0:单模场景   1:双模场景(暂时只支持HIRF下的L+NR) */
...
typedef struct {
    ...
    OM_COMPONENT_MODE_STRU stComponentId; /* 和原结构与原定义一致，
