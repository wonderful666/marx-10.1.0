	...
	loff_t *off)
{
	struct ivp_ipc_device *pdev =
		(struct ivp_ipc_device *)fd->private_data;
...
/* trans ion fd to phyaddr */
...
#endif
		{
			ret = ivp_trans_sharefd_to_phyaddr(pdev,
	...
	if (tmp_buff[0] == 0) {
		ivp_info("receive ipc cmd 0x%x from node:%s",
			((unsigned char)tmp_buff[3] & TMP_BUFF_MASK), pdev->device.name); // 3 is array element
	}
...
static int ivp_ipc_release(struct inode *inode __attribute__((unused)), struct file *fd)
{
	struct ivp_ipc_device *pdev =
		(struct ivp_ipc_device *)fd->private_data;
		...
		struct ivp_ipc_device *pdev)
{
	...
	ivp_info("ipc device %s init sucess", pdev->device.name);
	return ret;
...
static int ivp_ipc_probe(struct platform_device *platform_pdev)
{
	...
	ivp_info("ipc device %s probe sucess", pdev->device.name);
	return ret;
}
...
{
	struct ivp_ipc_device *pdev =
		(struct ivp_ipc_device *)platform_get_drvdata(plat_devp);
