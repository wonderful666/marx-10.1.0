...
int hispcpe_powerup(struct hispcpe_s *dev)
{
	...
	unsigned long transition_dest_rate0_denve = 406000000;
	unsigned long transition_dest_rate1_denve = 325000000;
	unsigned long transition_dest_rate2_denve = 232000000;
	unsigned long transition_dest_rate3_denve = 102000000;
	unsigned long transition_trans_rate_denve = 172000000;
	...
	if (dev->version == IPP_PHOENIX) {
		...
		if ((ret = jpeg_ipp_set_transition_rate(transition_source_rate, transition_dest_rate0_denve,
				transition_trans_rate_denve)) != true) {
			pr_err("[%s] Failed: rate0.%d\n", __func__, ret);
			goto fail_enjgpclk;
		}

		if ((ret = jpeg_ipp_set_transition_rate(transition_source_rate, transition_dest_rate1_denve,
				transition_trans_rate_denve)) != true) {
			pr_err("[%s] Failed: rate1.%d\n", __func__, ret);
			goto fail_enjgpclk;
		}

		if ((ret = jpeg_ipp_set_transition_rate(transition_source_rate, transition_dest_rate2_denve,
				transition_trans_rate_denve)) != true) {
			pr_err("[%s] Failed: rate2.%d\n", __func__, ret);
			goto fail_enjgpclk;
		}

		if ((ret = jpeg_ipp_set_transition_rate(transition_source_rate, transition_dest_rate3_denve,
				transition_trans_rate_denve)) != true) {
...
static int hispcpe_probe(struct platform_device *pdev)
{
	...
	if ((dev->sec_slamdrv = hipp_register(HISI_ORB_UNIT, NONTRUS)) == NULL) {
		pr_err("[%s] Failed : slam_register.%pK\n", __func__, dev->sec_slamdrv);
		goto free_iommu;
	}

	if ((dev->rdrdrv = hipp_register(HISI_REORDER_UNIT, NONTRUS)) == NULL) {
		pr_err("[%s] Failed : reorder_register.%pK\n", __func__, dev->rdrdrv);
		goto free_iommu;
	}

	if ((dev->cmpdrv = hipp_register(HISI_COMPARE_UNIT, NONTRUS)) == NULL) {
