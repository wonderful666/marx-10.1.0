...
static bool hifi_misc_local_process(unsigned short _msg_id)
{
	...
	case ID_HIFI_AP_SMARTPA_DFT_PROCESS_ERROR:
	case ID_HIFI_AP_SMARTPA_DFT_F0_ERROR:
	case ID_HIFI_AP_SMARTPA_DFT_RESISTANCE_ERROR:
	case ID_HIFI_AP_SMARTPA_DFT_OVERHEAT_ERROR:
...
static void hifi_misc_mesg_process(void *cmd)
{
	...
	case ID_HIFI_AP_AUXHEAR_CMD:
		...
		break;
	case ID_HIFI_AP_SMARTPA_DFT_PROCESS_ERROR:
		hifi_om_rev_data_handle(HIFI_OM_WORK_SMARTPA_PROCESS_ERROR,
			hifi_om_rev_data->data, hifi_om_rev_data->data_len);
		break;
	case ID_HIFI_AP_SMARTPA_DFT_F0_ERROR:
		hifi_om_rev_data_handle(HIFI_OM_WORK_SMARTPA_F0_ERROR,
			hifi_om_rev_data->data, hifi_om_rev_data->data_len);
		break;
	case ID_HIFI_AP_SMARTPA_DFT_RESISTANCE_ERROR:
		hifi_om_rev_data_handle(HIFI_OM_WORK_SMARTPA_RESISTANCE_ERROR,
			hifi_om_rev_data->data, hifi_om_rev_data->data_len);
		break;
	case ID_HIFI_AP_SMARTPA_DFT_OVERHEAT_ERROR:
		hifi_om_rev_data_handle(HIFI_OM_WORK_SMARTPA_OVERHEAT_ERROR,
			hifi_om_rev_data->data, hifi_om_rev_data->data_len);
		break;
	case ID_HIFI_AP_SYSCACHE_QUOTA_CMD:
