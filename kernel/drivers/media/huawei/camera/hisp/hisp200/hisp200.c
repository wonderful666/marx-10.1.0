...
void hisp200_destroy_timestamp(void)
{
	...
	rc = memset_s(&s_timeval, sizeof(struct timeval), 0x00, sizeof(struct timeval));
	if (rc != EOK) {
...
void hisp200_handle_msg(hisp_msg_t *msg)
{
	...
	case BATCH_REQUEST_RESPONSE:
		...
		hisp200_set_timestamp(&(msg->u.ack_batch_request.timestampH), &(msg->u.ack_batch_request.timestampL));
		break;
...
static int hisp200_unmap_a7isp_addr(void *cfg)
{
	...
	cam_info("func %s: a7 %x, size %x",__func__,
...
static int hisp200_init_r8isp_memory_pool(void *cfg)
{
	...
	cam_info("%s: type=%d, prot=0x%x sec=0x%x", __func__,
		pcfg->param.type, pcfg->param.prot, pcfg->param.security_isp_mode);
...
static int hisp200_deinit_r8isp_memory_pool(void *cfg)
{
	...
	rc = memset_s(&(s_hisp200.mem_pool[ipool]), sizeof(struct hisp200_mem_pool),
...
static int hisp200_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_ISP_TURBO:
		...
		rc = hisp_set_clk_rate(ISPFUNC_CLK, ISP_TURBO_ISPFUNC_CLK_RATE);
		rc = hisp_set_clk_rate(ISPFUNC2_CLK, ISP_TURBO_ISPFUNC2_CLK_RATE);
		rc = hisp_set_clk_rate(ISPFUNC3_CLK, ISP_TURBO_ISPFUNC3_CLK_RATE);
		rc = hisp_set_clk_rate(ISPFUNC4_CLK, ISP_TURBO_ISPFUNC4_CLK_RATE);
		break;
	case HISP_CONFIG_ISP_NORMAL:
		cam_debug("%s HISP_CONFIG_ISP_NORMAL", __func__);
		rc = hisp_set_clk_rate(ISPFUNC_CLK, ISP_NORMAL_ISPFUNC_CLK_RATE);

		isp_func2_clk = hisp_get_clk_rate(ISPFUNC2_CLK);
		if (isp_func2_clk == ISP_NORMAL_ISPFUNC2_CLK_URBO_RATE)
			rc = hisp_set_clk_rate(ISPFUNC2_CLK, ISP_NORMAL_ISPFUNC2_CLK_NORMAL_RATE);
		rc = hisp_set_clk_rate(ISPFUNC2_CLK, ISP_NORMAL_ISPFUNC2_CLK_LOW_RATE);
		rc = hisp_set_clk_rate(ISPFUNC3_CLK, ISP_NORMAL_ISPFUNC3_CLK_RATE);
		rc = hisp_set_clk_rate(ISPFUNC4_CLK, ISP_NORMAL_ISPFUNC4_CLK_RATE);
		break;
	case HISP_CONFIG_ISP_LOWPOWER:
		cam_debug("%s HISP_CONFIG_ISP_LOWPOWER", __func__);
		rc = hisp_set_clk_rate(ISPFUNC_CLK, ISP_LOWPOWER_ISPFUNC_CLK_RATE);

		isp_func2_clk = hisp_get_clk_rate(ISPFUNC2_CLK);
		if (isp_func2_clk == ISP_LOWPOWER_ISPFUNC2_CLK_TURBO_RATE)
			rc = hisp_set_clk_rate(ISPFUNC2_CLK, ISP_LOWPOWER_ISPFUNC2_CLK_NORMAL_RATE);
		rc = hisp_set_clk_rate(ISPFUNC2_CLK, ISP_LOWPOWER_ISPFUNC2_CLK_LOW_RATE);
		rc = hisp_set_clk_rate(ISPFUNC3_CLK, ISP_LOWPOWER_ISPFUNC3_CLK_RATE);
		rc = hisp_set_clk_rate(ISPFUNC4_CLK, ISP_LOWPOWER_ISPFUNC4_CLK_RATE);
		break;
...
int hisp200_release_sec_fw_buffer(void)
{
	...
	memset_s(&(s_hisp200.mem), sizeof(struct isp_mem), 0, sizeof(struct isp_mem));
	mutex_unlock(&hisp_mem_lock_mutex);
	return rc;
}
