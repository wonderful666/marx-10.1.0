...
static void hisp250_save_rpmsg_data(const void *data, int len)
{
	...
	if (memcpy_s(skbdata, len, data, len) != 0) {
		len = -EFAULT;
		cam_err("Fail: %s()%d ret = %d", __func__, __LINE__, len);
	}
...
static int hisp250_deinit_r8isp_memory_pool(void *cfg)
{
	...
	memset_s(&(s_hisp250.mem_pool[ipool]), sizeof(struct hisp250_mem_pool), 0, sizeof(struct hisp250_mem_pool));
	mutex_unlock(&hisi_rpmsg_service_mutex);
...
int hisp250_release_sec_fw_buffer(void)
{
	...
	memset_s(&(s_hisp250.mem), sizeof(struct isp_mem), 0, sizeof(struct isp_mem));
	mutex_unlock(&hisp_mem_lock_mutex);
	return rc;
}
static void hisp250_deinit_isp_mem(void)
