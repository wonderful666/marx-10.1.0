...
static int
hisp120_rpmsg_ept_cb(struct rpmsg_device *rpdev,
...
static int hisp120_get_a7isp_addr(void *cfg)
{
	...
	ret = hisp_get_sg_table(pcfg->param.sharedFd,&(s_hisp120.pdev->dev),&buf,&attach,&table);
	if(ret < 0){
...
static int hisp120_deinit_r8isp_memory_pool(void *cfg)
{
    ...
    if (s_hisp120.mem_pool[ipool].active) {
        ...
        hicam_buf_put_sgtable(s_hisp120.mem_pool[ipool].sgt);
    }
    rc = memset_s(&(s_hisp120.mem_pool[ipool]), sizeof(struct hisp120_mem_pool),
...
static int
hisp120_rpmsg_driver_cb(struct rpmsg_device *rpdev,
            void *data, int len, void *priv, u32 src)
{
    cam_info("%s enter ....\n", __func__);
...
hisp120_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
    ...
    if (EOK != memcpy_s(user_addr, sizeof(hisp_msg_t), msg, rc)) {
        rc = -EFAULT;
