...
struct rpmsg_service_info {
	struct rpmsg_hisp110_service *hisi_isp_serv;
...
static int hisp110_recv_rpmsg(hisp_intf_t *i,
...
static int
hisp110_rpmsg_ept_cb(struct rpmsg_device *rpdev,
...
static int hisp110_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		if (!remote_processor_up) {
			cam_notice("%s power on the hisp110.\n", __func__);
			rc = hisp110_power_on(i);
		}
		else{
			cam_warn("%s hisp110 is still on power-on state, power off it.\n",
				__func__);
	...
	case HISP_CONFIG_POWER_OFF:
		if (remote_processor_up) {
			cam_notice("%s power off the hisp110.\n", __func__);
			rc = hisp110_power_off(i);
		}
		break;
...
static int hisp110_power_on(hisp_intf_t *i)
{
	...
	struct rpmsg_hisp110_service *hisi_serv = NULL;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0))
	hisi_serv->ept =
		rpmsg_create_ept(hisi_serv->rpdev, hisp110_rpmsg_ept_cb, hisi_serv, chinfo);
#else
	hisi_serv->ept =
		rpmsg_create_ept(hisi_serv->rpdev, hisp110_rpmsg_ept_cb, hisi_serv,
				 RPMSG_ADDR_ANY);
#endif
	if (!hisi_serv->ept) {
...
#endif
{
	struct rpmsg_hisp110_service *hisi_serv = dev_get_drvdata(&rpdev->dev);
...
static int
hisp110_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
	int rc = 0;
	hisp110_t *hi = NULL;
	struct rpmsg_hisp110_service *hisi_serv = NULL;
	...
	if ((NULL == i) || (NULL == from_user || (0 >= len))){
		cam_err("func %s: from_user(%pK) i(%pK) len(%zu)",__func__, from_user, i, len);
		return -1;
	}
	hi = I2HI(i);
...
static int
hisp110_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
	int rc = len;
	hisp110_t *hi = NULL;
	struct rpmsg_hisp110_service *hisi_serv = NULL;
