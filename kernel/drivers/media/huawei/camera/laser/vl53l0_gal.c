...
static int vl53l0_gal_get_dt_data(struct platform_device *pdev, hw_vl53l0_t *laser)
{
    ...
    void *arg)
{
    ...
    if (NULL == sd) {
        cam_err("%s, sd for gal laser is NULL \n", __func__);
        return -EINVAL;
    }

    if (NULL == arg) {
        cam_err("%s, arg for gal laser is NULL \n", __func__);
        return -EINVAL;
    }
...
static int32_t vl53l0_platform_probe(struct platform_device *pdev)
{
    ...
    //get dt data
    if (pdev->dev.of_node) {
        rc = vl53l0_gal_get_dt_data(pdev, laser);
        if (rc < 0) {
    ...
    //init gpio
    rc = gpio_request(laser->laser_info->laser_gpio[XSHUT].gpio, "vl53l0_gal");
    if (rc < 0) {
        cam_err("%s failed to request reset pin. gpio = %x",
                __func__,laser->laser_info->laser_gpio[XSHUT].gpio);
        rc = -EIO;
        goto vl53l0_probe_fail;
    }
...
static const struct of_device_id hw_vl53l0_dt_match[] = {
    {.compatible = "huawei,vl53l0_gal"},
    {}
};

MODULE_DEVICE_TABLE(of, hw_vl53l0_dt_match);
static struct platform_driver vl53l0_gal_platform_driver = {
    .driver = {
        .name = "vl53l0_gal",
        .owner = THIS_MODULE,
...
static int __init vl53l0_gal_mod_init(void)
{
    int rc = 0;
    cam_info(" Enter %s ", __func__);

    rc = platform_driver_probe(&vl53l0_gal_platform_driver, vl53l0_platform_probe);
    if (rc < 0) {
...
static void __exit vl53l0_gal_mod_exit(void)
{
    platform_driver_unregister(&vl53l0_gal_platform_driver);
    if (s_laser) {
...
module_init(vl53l0_gal_mod_init);
module_exit(vl53l0_gal_mod_exit);
