...
static unsigned int support_dual_led_type = DUAL_LED_TEMPERATURE;
...
static int hw_lm3646_torch_mode_mmi(struct hw_flash_ctrl_t *flash_ctrl, int data)
{
    struct hw_flash_i2c_client *i2c_client = NULL;
    struct hw_flash_i2c_fn_t *i2c_func = NULL;
    struct hw_lm3646_private_data_t *pdata = NULL;
    ...
    i2c_func->i2c_write(i2c_client, REG_MAX_CURRENT, pdata->ecurrent[CURRENT_TORCH_MIN_MMI]);
    /* set LED Flash current value */
    if (data == TORCH_LEFT_MODE) {
        cam_info("%s flash_led=TORCH_LEFT_MODE", __func__);
        /* REG_CURRENT_CONTROL[3:0] control flash current */
        val = ((val & TORCH_ENABLE) | (pdata->ecurrent[CURRENT_TORCH_MAX_LEVEL_MMI]));

    } else if (data == TORCH_RIGHT_MODE){
        cam_info("%s flash_led=TORCH_RIGHT_MODE", __func__);
        /* REG_CURRENT_CONTROL[3:0] control flash current */
        val = ((val & TORCH_ENABLE) | (pdata->ecurrent[CURRENT_TORCH_MIN_LEVEL_MMI]));
    }else {
...
static ssize_t hw_lm3646_lightness_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
