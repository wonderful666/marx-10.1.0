...
static void get_als_debug_para(als_para_normal_table *als_para_diff_tp_color_table,
		short *als_debug_para, int num)
{
	int8_t i;

	for (i = 0; i < num; i++)
		als_debug_para[i] =
			als_para_diff_tp_color_table[als_para_table].als_para[i + 1];

	hwlog_info("%s:stk32670_als_flag is true and als_para_table=%d\n",
		__func__, als_para_table);
}

static void modify_als_debug_para(als_para_normal_table *als_para_diff_tp_color_table,
		short *als_debug_para, int num)
{
	int8_t i;
	...
	if (sizeof(als_para_diff_tp_color_table[als_para_table].als_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE)
		return;

	memcpy(als_data.als_extend_data,
		als_para_diff_tp_color_table[als_para_table].als_para,
		sizeof(als_para_diff_tp_color_table[als_para_table].als_para));
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (rohm_rgb_flag == 1) { /* bh745_para */
		...
		hwlog_info("%s:tmd2745_flag is true and als_para_table=%d\n",
			__func__, als_para_table);
	} else if (stk32670_als_flag) { /* stk32670_para */
		get_als_debug_para(stk32670_als_para, als_debug_para, sizeof(als_debug_para));
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (rohm_rgb_flag == 1) { /* bh745_para */
		...
		memcpy(als_data.als_extend_data,
			...
			SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
			sizeof(tmd2745_als_para_diff_tp_color_table[als_para_table].als_para));
	} else if (stk32670_als_flag) {
		modify_als_debug_para(stk32670_als_para, als_debug_para, sizeof(als_debug_para));
	}
	return size;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ret > 0) {
		if (als_ud_need_reset) {
			als_ud_need_reset = 0;
			head = als_under_tp_cal_data.x - HALF_LENGTH > 0 ?
				als_under_tp_cal_data.x - HALF_LENGTH : 0;
			left = als_under_tp_cal_data.y - HALF_LENGTH > 0 ?
				als_under_tp_cal_data.y - HALF_LENGTH : 0;
			bottom = als_under_tp_cal_data.x + HALF_LENGTH;
			right = als_under_tp_cal_data.y + HALF_LENGTH;
			return snprintf(buf, MAX_STR_SIZE, "%d,%llu,%llu\n",
...
static int als_ud_reset_immediately(void)
{
	hwlog_info("als_ud_reset_immediately in\n");
	if (sem_als_ud_rgbl_block_flag == 1) {
		send_als_under_tp_calibrate_data_to_mcu();
		als_ud_rgbl_block = 1;
	...
	char *buf)
{
	...
	if (als_ud_cali_xy[0] == 0)
		...
		return snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1,
			"%d, %d, %d, %d, %d\n",
			als_support_under_screen_cali,
			als_ud_cali_xy[0], als_ud_cali_xy[1],
