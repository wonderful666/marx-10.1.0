...
#include <huawei_platform/power/adapter/adapter_protocol_scp.h>
...
static const struct adapter_protocol_device_data g_scp_protocol_dev_data[] = {
...
/* power: 10^n */
static int g_scp_protocol_ten_power[] = {
	[0] = 1,
	[1] = 10,
	[2] = 100,
	[3] = 1000,
...
static int scp_protocol_get_device_id(const char *str)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_scp_protocol_dev_data); i++) {
		if (!strncmp(str, g_scp_protocol_dev_data[i].name,
			strlen(str)))
			return g_scp_protocol_dev_data[i].id;
	}
...
static int scp_protocol_check_adp_type(int mode, int value)
{
	...
	if ((mode & ADAPTER_SUPPORT_SCP_B_LVC) &&
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_QTR_C_60W:
			return ADAPTER_TYPE_QTR_C_20V3A;
		case SCP_PROTOCOL_ADP_B_TYPE1_QTR_C_40W:
			return ADAPTER_TYPE_QTR_C_10V4A;
		case SCP_PROTOCOL_ADP_B_TYPE1_QTR_A_40W:
			return ADAPTER_TYPE_QTR_A_10V4A;
		case SCP_PROTOCOL_ADP_B_TYPE1_QTR_A_22P5W:
	...
	if (mode & ADAPTER_SUPPORT_SCP_B_SC) {
		...
		/* check 10v2.25a-car */
		case SCP_PROTOCOL_ADP_B_TYPE1_22P5W_CAR:
...
static int scp_protocol_get_min_voltage(int *volt)
{
	...
	*volt = g_scp_protocol_ten_power[value_a] * value_b;
...
static int scp_protocol_get_min_current(int *cur)
{
	...
	if (drop_flag == SCP_PROTOCOL_DROP_ENABLE)
		*cur = g_scp_protocol_ten_power[value_a] * value_b *
...
static int scp_protocol_get_extend_current(void)
{
	int value[BYTE_ONE] = {0};

	if (!scp_protocol_reg_read(SCP_PROTOCOL_MAX_IOUT_EXTEND, value, BYTE_ONE))
		return g_scp_protocol_ten_power[1] * value[0]; /* 1: 10^1 */
...
static int scp_protocol_get_max_current(int *cur)
{
	...
	*cur = g_scp_protocol_ten_power[value_a] * value_b;
	if (*cur >= SCP_PROTOCOL_MAX_IOUT_EXTEND_TH)
		*cur += scp_protocol_get_extend_current();
...
static int scp_protocol_get_power_drop_current(int *cur)
{
	int drop_flag;
	int drop_ratio;
	...
	if ((drop_flag == SCP_PROTOCOL_DROP_ENABLE) && (drop_ratio > 0)) {
		*cur = *cur * drop_ratio / SCP_PROTOCOL_DROP_FACTOR;
...
static int scp_protocol_get_device_info(struct adapter_device_info *info)
{
	if (!info)
		return -1;
...
static struct adapter_protocol_ops adapter_protocol_scp_ops = {
	...
	.get_power_drop_current = scp_protocol_get_power_drop_current,
