...
static int direct_charge_get_power_drop_current(int *value)
{
	struct direct_charge_device *l_di = direct_charge_get_di();
...
static int direct_charge_get_adapter_max_current_by_volt(int val,
...
static int direct_charge_get_adapter_max_current_by_adapter_type(int value)
{
	int max_cur;
	struct direct_charge_device *l_di = direct_charge_get_di();
	...
	max_cur = direct_charge_get_adapter_max_current_by_volt(value, l_di);
	if (max_cur == 0)
		direct_charge_get_adapter_max_current(&max_cur);
...
static void direct_charge_send_icon_uevent(void)
{
	...
	adp_limit = direct_charge_get_adapter_max_current_by_adapter_type(
...
static void direct_charge_get_max_power(void)
{
	...
	/* get max current by adapter type */
	adp_limit = direct_charge_get_adapter_max_current_by_adapter_type(
		bat_vol * l_di->dc_volt_ratio);
	adp_limit *= l_di->dc_volt_ratio;
...
static bool direct_charge_is_priority_inversion(void)
{
	struct direct_charge_device *lvc_di = NULL;
	struct direct_charge_device *sc_di = NULL;
...
int direct_charge_check_adapter_antifake(void)
{
	...
	max_cur = direct_charge_get_adapter_max_current_by_adapter_type(
		bat_vol * l_di->dc_volt_ratio);
	if (max_cur == 0)
		return -1;
	...
	if (l_di->working_mode == LVC_MODE) {
		n_data.event = "AUTH_DCCT_LVC=";
		n_data.event_len = 14; /* length of AUTH_DCCT_LVC= */
	} else {
...
static int direct_charge_select_adp_max_curr(struct direct_charge_device *di)
{
	...
	adp_max_cur = direct_charge_get_adapter_max_current_by_adapter_type(
		di->adaptor_vset);
