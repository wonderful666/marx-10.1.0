...
static int nu1619_i2c_read(struct i2c_client *client,
...
static int nu1619_i2c_write(struct i2c_client *client,
...
static u8 *nu1619_get_chip_info_str(void)
{
	int ret;
	struct nu1619_chip_info chip_info;
	static u8 chip_info_str[NU1619_CHIP_INFO_STR_LEN] = { 0 };

	ret = nu1619_get_chip_info(&chip_info);
	if (ret)
		return "error read";

	memset(chip_info_str, 0, NU1619_CHIP_INFO_STR_LEN);

	snprintf(chip_info_str, NU1619_CHIP_INFO_STR_LEN,
		"chip_id:nu%x fw:%x\n", chip_info.chip_id,
		chip_info.mtp_fw_ver);
...
static u8 *nu1619_get_die_id(void)
{
	return "nu1619";
}
...
static void nu1619_ext_pwr_post_ctrl(int flag)
{
	...
	usleep_range(9500, 10500); /* delay 10ms for power stablity */
...
static int nu1619_send_msg_ack(u8 cmd, u8 *data, int data_len)
{
	...
	do {
		nu1619_send_msg(cmd, data, data_len);
		for (ack_cnt = 0;
...
static char *nu1619_get_rx_fod(void)
{
	int i;
	int ret;
	char tmp[NU1619_RX_FOD_TMP_STR_LEN] = { 0 };
	u8 fod_arr[NU1619_RX_FOD_LEN] = { 0 };
	static char fod_str[NU1619_RX_FOD_STR_LEN] = { 0 };

	if (NU1619_RX_FOD_STR_LEN <=
		NU1619_RX_FOD_TMP_STR_LEN * NU1619_RX_FOD_LEN)
		return "space error";

	memset(fod_str, 0, NU1619_RX_FOD_STR_LEN);
...
static int nu1619_set_rx_fod(char *fod_str)
{
	int i;
	int ret;
	u8 val = 0;
	...
	if (!fod_str) {
		hwlog_err("set_rx_fod: input fod_str null\n");
		return -WLC_ERR_PARA_NULL;
	}
...
static void nu1619_rx_power_on_handler(struct nu1619_dev_info *di)
{
	...
	nu1619_read_cmd_data(NU1619_REG_CMD_RX_SS, &rx_ss,
		NU1619_CMD_REG_BYTE_LEN);
...
static int nu1619_tx_chip_init(void)
{
	int ret;
...
static void nu1619_handle_tx_ept(struct nu1619_dev_info *di)
{
	...
	case NU1619_TX_EPT_CMD:
		di->ept_type &= ~NU1619_TX_EPT_CMD;
		hwlog_info("[handle_tx_ept] ept command\n");
		break;
...
static struct wireless_charge_device_ops nu1619_rx_ops = {
	...
	.get_rx_chip_id         = nu1619_get_chip_id,
	...
	.rx_program_otp         = nu1619_rx_program_mtp,
	.rx_check_otp           = nu1619_check_mtp,
	.check_is_otp_exist     = nu1619_check_is_mtp_exist,
};
...
static int nu1619_tx_ps_ops_register(void)
{
	...
	hwlog_err("tx_ps_ops_register: fail\n");
	return -WLC_ERR_MISMATCH;
}
	...
	const struct i2c_device_id *id)
{
	...
	qi_protocol_ops_register(&nu1619_qi_protocol_ops);
