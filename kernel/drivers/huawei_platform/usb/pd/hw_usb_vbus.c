...
#ifdef CONFIG_POGO_PIN
static struct notifier_block g_pogopin_charge_nb;
...
#ifdef CONFIG_POGO_PIN
static int pogopin_charge_event_notifier_call(struct notifier_block *usb_nb,
	unsigned long event, void *data)
{
	if (event == POGOPIN_VBUS_CONNECT) {
		hwlog_info("%s: pogo_vbus_in\n", __func__);
		send_charger_connect_event();
	} else if (event == POGOPIN_VBUS_DISCONNECT) {
		send_charger_disconnect_event();
		hwlog_info("%s: pogo_vbus_out\n", __func__);
	}
}
#endif /* CONFIG_POGO_PIN */
...
#ifdef CONFIG_POGO_PIN
	g_pogopin_charge_nb.notifier_call = pogopin_charge_event_notifier_call;
	ret = pogopin_event_notifier_register(&g_pogopin_charge_nb);
	if (ret < 0)
