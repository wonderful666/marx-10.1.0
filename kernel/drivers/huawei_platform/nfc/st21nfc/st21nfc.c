...
/*
	if ( ((pinlev > 0) && (pdev->platform_data.active_polarity == 0))
	    || ((pinlev == 0)
		&& (pdev->platform_data.active_polarity == 1))) {
	...
	for (retry = 0; retry < NFC_TRY_NUM; retry++) {
	    ret = i2c_master_recv(pdev->platform_data.client, buf, count);
		for (i = 0; i < count; i++) {
			snprintf(&tmpStr[i * 2], 3, "%02X", buf[i]);
		}
		hwlog_info("%s : retry = %d, ret = %d, count = %3d > %s\n", __func__, retry, ret, (int)buf[2]+2, tmpStr);		
		if (ret == (int)count) {
			isSuccess = true;
			break;
		} else {
