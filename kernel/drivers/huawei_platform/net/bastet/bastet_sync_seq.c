 ...
 * Description: Bastet Tcp Seq Sync.
 *
...
static int request_sock_sync_ipv4(struct sock *sk)
{
	...
	bastet_get_comm_prop(sk, &prop);
	err = post_indicate_packet(BST_IND_SOCK_SYNC_REQ,
		&prop, sizeof(prop));
	...
	struct bst_set_sock_sync_prop_ipv6 *set_prop)
{
	...
	if (bsk->sync_p != NULL) {
		bastet_loge("proxy_id:%d has a pending sock set",
			bsk->proxy_id);
		err = -EPERM;
		goto out_unlock;
	}
	...
	struct bst_sock_comm_prop_ipv6 *comm_prop)
{
	...
	if (sk == NULL) {
		bastet_loge("ipv6 can not find sock by lport:\
			%d, lIp: ***, rport: %d, rIp: ***",
			comm_prop->local_port,
			comm_prop->remote_port);
		return -ENOENT;
