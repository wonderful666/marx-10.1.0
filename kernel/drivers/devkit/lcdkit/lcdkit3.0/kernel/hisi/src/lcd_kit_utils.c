...
#define FLASH_GAMMA_TOTAL_LEN 452
...
static int lcd_kit_flash_gamma_checksum(uint8_t gamma[], uint32_t length)
{
	uint32_t i;
	uint32_t sum_calc = 0;
	uint32_t sum_read;
	...
	// checksum rule: sum index from 0 to (FLASH_GAMMA_TOTAL_LEN - 3)
	...
	struct lcd_kit_adapt_ops *adapt_ops)
{
	...
	// read otp gamma
	if (adapt_ops->mipi_rx(hld, gamma_60hz,
	...
	void *hld, struct lcd_kit_adapt_ops *adapt_ops, uint8_t param[])
{
	...
	for (retry_count = 0; retry_count < FLASH_GAMMA_SINGLE_PARAM_RETRY_MAX;
		retry_count++) {
		if (adapt_ops->mipi_rx(hld, param,
...
{
	int index;
	uint8_t flash_single_param[3] = {0}; // rx cmd buffer size is 3
...
int lcd_kit_read_fps_gamma(void *hld)
{
	...
	if (ret != LCD_KIT_OK)
		LCD_KIT_WARNING("read_flash_exit failed! ret = \n", ret);
...
static int lcd_kit_write_fps_gamma_by_scene(struct hisi_fb_data_type *hisifd, const int index)
{
	int ret = LCD_KIT_OK;
	if (!disp_info->fps_gamma.support) {
		LCD_KIT_DEBUG("fps_gamma not support");
...
static int lcd_kit_updt_panel_fps(struct hisi_fb_data_type *hisifd, char fps_scence)
{
	...
	lcd_kit_write_fps_gamma_by_scene(hisifd, index);
	...
	if (common_info->dfr_info.fps_lock_command_support) {
		...
		wake_up_interruptible(&common_info->dfr_info.hbm_hist);
	}
	return ret;
}
...
static int lcd_kit_updt_mipi_fps(struct hisi_fb_data_type *hisifd, int scence)
{
	...
	if (pinfo->fps_updt != pinfo->fps && pinfo->fps_updt != 0) {
		LCD_KIT_ERR("the last fps %d update handle have not finished\n",
			pinfo->fps_updt);
		return LCD_KIT_FAIL;
	}
	/* enable fps update func */
	if (common_info->dfr_info.fps_lock_command_support) {
		wait_event_interruptible_timeout(common_info->dfr_info.fps_hist,
...
int lcd_kit_updt_fps_scence(struct platform_device *pdev, uint32_t scence)
{
	...
	if (lcd_is_support_dfr()) {
		...
		if (ret != LCD_KIT_OK)
			LCD_KIT_ERR("lcd_kit_updt_mipi_fps error %d\n", ret);
		return ret;
	}
...
static void lcd_kit_fps_init(struct hisi_panel_info *pinfo)
{
	if (lcd_is_support_dfr()) {
		pinfo->dfr_support = FPS_UPDATE_ENABLE;
...
void lcd_kit_parse_fps(struct device_node *np)
{
	...
	if (disp_info->fps.fps_switch_support)
	...
	u8 reg_val[DETECT_LOOPS][DETECT_NUM][VAL_NUM])
{
	...
	for (i = 0; i < DETECT_LOOPS; i++) {
		...
		ret = snprintf(err_info + len, DMD_DET_ERR_LEN,
			"%d: %x %x, %x %x, %x %x, %x %x ",
			i + DET_START,
...
static int lcd_kit_gpio_pcd_errflag_read(int gpio_no, int *read_value)
{
	int ret;

	if (gpio_no == 0) {
		LCD_KIT_ERR("gpio_no is 0!\n");
		return LCD_KIT_FAIL;
	}

	ret = gpio_request(gpio_no, GPIO_PCD_ERRFLAG_NAME);
	if (ret != 0) {
...
int lcd_kit_gpio_pcd_errflag_check(void)
{
	...
	u8 result_value = PCD_ERRFLAG_SUCCESS;

	if (!disp_info->pcd_errflag.pcd_errflag_check_support) {
		LCD_KIT_INFO("pcd_errflag check default pass\n");
		return PCD_ERRFLAG_SUCCESS;
	}
	ret = lcd_kit_gpio_pcd_errflag_read(pcd_gpio, &pcd_gpio_value);
	if (ret != 0) {
		LCD_KIT_ERR("gpio[%d] read fail!\n", pcd_gpio);
		return PCD_ERRFLAG_SUCCESS;
	}
	ret = lcd_kit_gpio_pcd_errflag_read(errflag_gpio, &errflag_gpio_value);
	if (ret != 0) {
		LCD_KIT_ERR("gpio[%d] read fail!\n", errflag_gpio);
		return PCD_ERRFLAG_SUCCESS;
	}
