...
void lcdkit_info_init(void* pdata)
{
    ...
    pinfo->dither_support = lcdkit_info.panel_infos.dither_support;
	...
	const struct hisi_fb_data_type *hisifd)
{
	...
	mipi_dsi0_base = hisifd->mipi_dsi0_base;
	if (mipi_dsi0_base == NULL) {
		LCDKIT_ERR("mipi_dsi0_base is null point\n");
		return;
	}

	dsi_cmd.dtype = cmd->dtype;
	dsi_cmd.vc =  cmd->vc;
	dsi_cmd.wait =  cmd->wait;
	dsi_cmd.waittype =  cmd->waittype;
	dsi_cmd.dlen =  cmd->dlen;
	dsi_cmd.payload = cmd->payload;
...
static void read_ddic_reg_interface(uint8_t *out, struct lcdkit_dsi_panel_cmds *cmds, struct hisi_fb_data_type *hisifd, int max_out_size)
{
    ...
    for (i = 0; i < cmds->cmd_cnt; i++) {
		if (cm == NULL) {
			LCDKIT_ERR("cmd is null pointer\n");
			cm++;
			continue;
		}
