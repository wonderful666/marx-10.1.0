...
static int focal_ft3528_get_adc_data(int *data, size_t size)
{
	...
	if (!read_data) {
		TS_LOG_ERR("%s:read_data buffer malloc fail\n", __func__);
		return -ENOMEM;
	}
	...
	if (ret) {
		TS_LOG_ERR("%s:read short data fail, ret=%d\n",
			__func__, ret);
		goto exit;
	}
	for (i = 1; i < packet_num; i++) {
		...
		if (ret) {
			TS_LOG_ERR("%s:read adc data error,ret = %d\n",
				__func__, ret);
			goto exit;
		}
	}
...
static int focal_ft3528_get_short_data(int *data, size_t size, u8 cmd)
{
	...
	if (!adc_ok) {
		TS_LOG_ERR("%s:adc data not ready", __func__);
		return ret;
	}

	ret = focal_ft3528_get_adc_data(data, size);
	if (ret) {
		TS_LOG_ERR("%s:get short adc data failed", __func__);
		return ret;
	}
	return ret;
}
	...
	unsigned int chl_x, unsigned int chl_y)
{
	...
	ret = focal_ft3528_get_short_data(offset_value, SHORT_TEST_OFFSET_SIZE,
		READ_ADC_CMD_OFFSET);
	if (ret) {
		TS_LOG_ERR("%s:failed to get weak short data,ret = %d",
			__func__, ret);
		goto free_mem;
	}
